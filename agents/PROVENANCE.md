...
## Fable 5 — 2026-08-23 (wave 2 — the plain-speech sweep, judged by quinn's own eyes)

| File | Author | Note |
|---|---|---|
| `orreth_sim/speech.py` · `console_worker.py` · `tests/test_speech.py` | Fable 5 (claude-fable-5) | Ten more Inbox kinds leave the code for the shelf as craft (passage reach-out · legacy · mind/service re-pins · entitlement · death attestation · dial turn · examiner degradation · charter rest · smith's nudge) — each names what it is, what each choice does, and what silence does; citations evicted from human cards; slot-honest (a mismatch card can never lie about its counts). Conformance 360. |
| `window.html` · `capabilities/*/genesis.py` | Fable 5 (claude-fable-5) | quinn-web walked the rendered glass post-sweep: 42 frictions, ZERO on the rewritten Inbox/Objectives. Her top clusters landed same-day: the "your word" idiom became plain action words ("needs your decision", "asks you first", "asking IS the approval"), DID/pin explained in place, the same-tool-many-floors note, THE DEAL/WHO-LEANS retitled in stranger-words, the desks' buttons rewritten ("watch it daily — you approve once, a fresh report repeats") — and her stranger-eyes caught a REAL copy bug: the options desk said "ask charles" where its specialist is chad. Report req-1165 keeps the design-level remainder at the architect's queue. |

## Fable 5 — 2026-08-25 (quinn v2 — the click-flow walk)
...
## Fable 5 — 2026-09-08 (0071 sp1–sp4 — the doors that answer, the build season opens)

| File | Author | Note |
|---|---|---|
| `orrethd/src/main.rs` · `contracts/v0/capability-token.schema.json` | Fable 5 (claude-fable-5) | sp1 — THE RESOLVE DOOR LOCKS: transitions are law (terminal statuses immutable; proved only from challenged); a resolution demands a root-chained token carrying the narrow `resolve` action on the `queue` space (JB's rule-9 word added both to the token grammar); the one tokenless lane left is the joiner answering becky's nonce, result keys held to {nonce, proof}; `/model/meter` demands the same lease `authorize` checks and only the subject's own line. Proven by the live forgery suite (three forgeries refused one-faced, the card unmoved; the pen resolves; a settled word never rewritten). |
| `orreth_sim/askdoor.py` · `tests/test_askdoor.py` · `orreth_sim/dispatcher.py` · SDK `client.py` | Fable 5 (claude-fable-5) | sp2 — THE ASK DOOR: one `kind:"ask"` for machines — signed with the asker's own key (never a bearer on the public queue), verified against the DID and the standing welcome, answered with a structured envelope whose refs are WHOLE; a present-but-invalid signature is a forgery in ANY mode (the walk's own find, closed the same hour); `dispatch(force=)` lets the asker choose the row with the choice still on the record; SDK `ask()`. Suite 379. |
| SDK `joindoor.py` · `orreth_sim/identity.py` (`Becky.adopt` · `mint_delegation`) · SDK `crypto.py` · `tests/test_delegation.py` | Fable 5 (claude-fable-5) | sp3 — THE PUBLISHABLE JOIN DOOR (KCR-0001 paid): the root signs a delegate's credential ONCE, offline, and never enters a serving process; the SDK door challenges, verifies, stages, and mints root-chained leases; the standing welcome became a signed record on the floor and becky's side writes the admission record. Proven by the stranger walk (a fresh world from the door + SDK alone); the walk's find — the clobbered approved-by line — fixed door and rig both. Suite 384. |
| `orreth_sim/traffic.py` · `tests/test_traffic.py` · `orreth_sim/dials.py` · `console_worker.py` · `orrethd/src/main.rs` · `smoke_orrethd.py` | Fable 5 (claude-fable-5) | sp4 — THE TRAFFIC LAW: fixed-window per-DID metering at both halves of the boundary (the plane's `knock()` on five doors; the worker's `do_POST` gate before the body is read), one honest busy face with a truthful retry hint, deliberate body ceilings (413) in place of framework accidents, two governed dials (`rate-per-min` 240 · `body-limit-kb` 2048), and the craft door's prod posture reaching `tool_invoke`. Proven live on a strict plane (5/min · 2000 bytes): [200×5, 429×3], the second identity unslowed, 413 loud; the default smoke re-ran whole. Suite 390. |
| `orreth_sim/askcache.py` · `tests/test_askcache.py` · `orreth_sim/askdoor.py` (`guardrails_version`) · `orreth_sim/dials.py` · `console_worker.py` | Fable 5 (claude-fable-5) | sp5 — THE ASK-CACHE: an answer serves again only for the same canonical words, variant, floor, and guardrail-set version (all four in the key — a changed guardrail set revalidates by construction); the cache is a PROJECTION — every entry leans on its signed exchange record and is checked alive read-side before any hit serves (purge reach with no hooks); every hit confessed in the envelope; the TTL a ladder-homed dial (ask-cache-ttl-s 300, 0 = closed); grace-served answers never cache. Proven live on the rig (hit confessed with the original exchange; canonicalization folding; the dial closing and restoring the cache through the one-motion door). Suite 398. |

## Fable 5 — 2026-09-09 (0065 sp1 — the variant registry, the E-RAG engine's first brick)

| File | Author | Note |
|---|---|---|
| `orreth_sim/variants.py` · `tests/test_variants.py` · `orreth_sim/dispatcher.py` · `console_worker.py` | Fable 5 (claude-fable-5) | THE ELEVEN AS DECLARATIONS (0065 §3.2, the 0063 pattern applied to flows): registry firmware (stages over six module boundaries · requires · cost class · delta · blast · genesis), `built()` derived from registry × standing flows (the three hand-kept arrays retire), `resolve()` speaking both vocabularies, the routing standard's genesis at v2 with comparative/temporal finally routed, the `variants` craft drawer (purpose by the razor; gate-checked before landing; teachings on every sibling), and the two-lane stitch (the universe pull keeps routing-standard rows). Proven live: a menu name served canonically over its legacy flow; declared-unbuilt fell loudly; undeclared refused naming the menu; the gate refused and landed at the door. Suite 407. |
| `orreth_sim/chunklaw.py` · `tests/test_chunklaw.py` · `orreth_sim/stacks.py` · `orreth_sim/rivals.py` · `orreth_sim/dials.py` · `console_worker.py` · `orrethd/src/pg.rs` · `orrethd/src/main.rs` | Fable 5 (claude-fable-5) | sp2 — THE STANDING CHUNK/TREE PROJECTION (L2's gate): the chunks table + three tokened doors + tombstone eviction on the plane (the embeddings lifecycle verbatim); the ONE knife (lane law extracted from project() — rebuild and sweep identical by construction); the pure chunk law (deterministic cut, lossless spans, piece hashes, policy naming, tree parents); chunk_beat on the chunk-every dial with per-floor clocks; the naive row's production path reads the shelf with hash re-checks, the rebuild standing behind as dev fallback. The first live sweep caught its own wound (global worklist judged unseen records) — fixed in the hour. Proven live: 253 cut in one beat; «standing shelf served 4 hit(s) — no rebuild». Suite 416. |
| `orreth_sim/graphlaw.py` · `tests/test_graphlaw.py` · `orreth_sim/rivals.py` · `orreth_sim/dials.py` · `console_worker.py` · `orrethd/src/pg.rs` · `orrethd/src/main.rs` | Fable 5 (claude-fable-5) | sp3 — THE GRAPH PROJECTION (Shape-A paid): graph_nodes + graph_edges in the always-on DDL (pure SQL — stands where the meaning axis is dark), every edge witnessed (record · span · hash) and law-versioned (extractor × chunk policy); three tokened doors with the walk confined to the caller's authorized ids; tombstone eviction; ONE term law (graphlaw.terms, rivals delegates by identity); extract_beat on the extract-every dial (per-floor clocks, driving from its own pull — sp2's lesson born in); the graph row's production path walks Postgres with the hash drift-catch. Proven live: 253 extracted in one sweep; «the graph walked 4 witness(es) in Postgres — no rebuild», variant confessed. Suite 424. |
| `orreth_sim/styles.py` · `tests/test_variant_*.py` (×5) · `orreth_sim/variants.py` · `orreth_sim/tournament.py` · `orreth_sim/rivals.py` · `orreth_sim/stacks.py` · `console_worker.py` | Fable 5 (claude-fable-5) | sp4 — THE FIVE NEW STYLES + THE ELEVEN-ARM TOURNAMENT: Hierarchical (section-climbing via the tree law) · Reasoning-First (the plan as a signed record, steps named) · Memory-Augmented (the chronicle expands the ask, confessed) · HyDE (search by the imagined answer — pseudo-relevance feedback, named honestly) · Corrective (the visible second look); FLAVORS derives from the registry (the hard-coded 7 dies; router retires from the standings); latency + cost axes paid into grade()/run() (0038 §4); config_for threads every row and both standing lanes — the Workshop's knobs take effect. Walk-finds paid in-hour: the plan-echo law (machinery never answers) and the pull's missing variant craft. Proven live: five styles served as themselves; k 4→2→4 through the door with the citation count following. Suite 450. |
| SDK `client.py` · `chassis.py` · `mind.py` · `console_worker.py` | Fable 5 (claude-fable-5) | sp5 — IDENTITY EVERYWHERE: the meter line wears the variant (client.meter grew variant=, GovernedThink threads it, the plane keeps it verbatim — per-style cost is a query); _variant rides the _klass kwarg seam (per-call attribution, absent = Auto); the exchange record wears variant:<style> as a tag (the arm-tag pattern at the style layer); the warmth tap expires by the recall-days dial (declared retention — a purged record's residue dies on the dial's clock; the reader was always alive-gated). Proven live from Postgres: the hyde meter line and the variant:graph tags read back; the glass spoke the selection in the parlor's own sentence. Suites 450 + SDK 24. |

## Fable 5 — 2026-09-09 (0066 sp1 — the join: the router's reward road)

| File | Author | Note |
|---|---|---|
| `orreth_sim/dispatcher.py` · `tests/test_choice_join.py` · `console_worker.py` | Fable 5 (claude-fable-5) | 0066 sp1 — THE CHOICE JOINS THE WORLD: dispatch records derive from the exact rulebook version that made them (empty lineage on genesis-only worlds — never fabricated), wear coordinate tags (variant:<style> · origin:<asker>), and name featurizer shapes-v0 + the whole standard_ref; the ask door's exchange derives from the choice (one GIN-indexed hop — the experiment's proven join at the routing layer); the parlor's routing note stops truncating the ref (0052's law; the recon's named wound); the yardstick rides the structured lane with the whole choice ref per row. Proven live: one SQL join walked exchange → choice on the rig; the choice's lineage resolved to the standard asset. Suite 454. |
| `orreth_sim/featurizer.py` · `tests/test_featurizer.py` · `orreth_sim/dispatcher.py` · `orreth_sim/askdoor.py` · `console_worker.py` | Fable 5 (claude-fable-5) | 0066 sp2 — THE ANALYZER: the declared feat-v1 extractor (kind · parts · length · domain · freshness · modality · latency pref · confidence, plain words, pinned per choice); the escalation the 0038 docstring promised — one governed thought on a LOW read from a non-fast asker (0048's pattern: typed contract over the menu, one re-ask, honest park), the asker's force outranking the mind, the mind's pick obeying the built law; latency the FIRST honored attribute (the envelope's honored list speaks); features + consult whole on the record. Proven live three lanes (consult chose multi-agent with its why; fast skipped the thought; clear stayed hot) — and the park law fired unprompted on a pin/class mismatch, serving exactly as designed. Suite 463. |
| `orreth_sim/signals.py` · `tests/test_signals.py` · `orreth_sim/dispatcher.py` · `orreth_sim/chassis.py` · `orreth_sim/vera.py` · `console_worker.py` | Fable 5 (claude-fable-5) | 0066 sp3 — SIGNALS AND THE RUNGS: the cheap bench at answer time (declared proxies: retrieval · faithfulness · answer · coverage) written into the exchange with latency + context cost; vector_for walks the records (verdicts + thumb by their own refs — never a second store); reward() = governed arithmetic with weights IN THE STANDARD (gated re-weighting; re-scorable history); vera's tier replaces the answer proxy; L3 AS CODE (thumb never a scalar — identical score up or down, 👎 vetoes); the Observation rung built (chassis observations scribe-signed, coordinate-bearing) and vera's WORK_TAGS grown to all three rungs. Proven live: envelope latency/context + the exchange's whole bench read back. Suite 471. |
| `orreth_sim/standings.py` · `tests/test_standings.py` · `console_worker.py` | Fable 5 (claude-fable-5) | 0066 sp4 — STANDINGS AND THE PROPOSAL GENERATOR: the Stage-1 scoreboard as a pure projection (Beta posteriors w/ credible intervals, approximation named; rebuildable; purge-reached; vetoes refused from the arithmetic — L3's third appearance in code); the replay harness (every unchosen arm panelled by the cheap bench, one signed record per ask naming its exchange); the generator's conservative test (challenger floor > incumbent ceiling, both at volume) staging onto 0038's own promotion road at the human's gate — thin evidence and unmeasured incumbents refuse; standings_beat breathing per floor. Proven live: first breath replayed + 11 cells + the REFUSAL holding; suite proves the staging side. Suite 478. |
| `orreth_sim/dials.py` · `orreth_sim/dispatcher.py` · `orreth_sim/standings.py` · `orreth_sim/tournament.py` · `tests/test_live_slice.py` · `console_worker.py` | Fable 5 (claude-fable-5) | 0066 sp5 — THE LIVE SLICE AND THE DIALS: dialed deterministic exploration (content-hash split, always confessed, never over the asker or a spent thought) inside a FIRMWARE 20% bound the dial can never widen; three router dials (replay-every · standings-min-n · live-slice-pct); propose() honors the dialed floor; the PUT side paid (registry+modality-derived, never "everything"); the router row retired honorably (one librarian-signed record — Auto absorbed it, the tactic lives in the swarm; planted once, idempotent). Proven live: the floor's gate refused a universe dial (the ladder law unprompted), the turn landed at the universe's door, the slice explored and confessed, canon restored; the rest read back from Postgres. Suite 484. THE DIVE CLOSES WHOLE — five spoonfuls, one day. |

## Fable 5 — 2026-09-09 (0067 sp1 — the act-graph format and the pen)

| File | Author | Note |
|---|---|---|
| `orreth_sim/actgraph.py` · `tests/test_actgraph.py` · `console_worker.py` · `orrethd/src/window.html` | Fable 5 (claude-fable-5) | 0067 sp1 (KCR-0004's payment begins) — THE FORMAT: act-v1 (v1-earmarked, sacred v0 untouched) with the four laws in validate() (every node a DOOR or refused · the narrative a bijection with silent boxes named · edges bind existing nodes · glow list kept); from_choreography converts the walk with nothing lost; all three worker walk sites emit it. THE PEN: the glass drawer renders act-v1 (labels + door-clicks through the standing reader; old payloads untouched — L1). THE APERTURE DOOR OPENS: /aperture fetches the signed envelope by coordinate across the floors; the walk detail shows THE ENVELOPE whole. Proven live: a staged plan validated flawless on the wire (then cancelled politely); a real July aperture served by coordinate. Walk-find: :4562's allow-list gates new doors. Suite 489. |
| `orreth_sim/actgraph.py` · `tests/test_actgraph.py` · `console_worker.py` · `orrethd/src/window.html` | Fable 5 (claude-fable-5) | 0067 sp2 — THE CAPABILITY FLOW MIGRATES: dkCvNode finally DEFINED (a run stage opens its own record; an unrun one confesses on an honest card — never a fake ref); stage rows carry their refs at the desk compose; actgraph.from_manifest_flow converts the second dialect (record doors for run stages, room doors for unrun, groups surviving, pipelines lawfully narrative-free); renderAct dispatches act-v1 pipelines to the serpentine lane (dkCanvas's earned goods intact) with actFromFlow as the glass twin. Proven live: 48/48 stage rows doored; one door walked end to end to f:charles's real record. Suite 490. |
| `orreth_sim/actgraph.py` · `tests/test_actgraph.py` · `console_worker.py` · `orrethd/src/window.html` | Fable 5 (claude-fable-5) | 0067 sp3 — THE ATLAS AND THE ESTATE MIGRATE: DOOR_KINDS absorbs the atlas grammar whole; from_atlas (legacy keys ride inside the act door — L1 both directions; atlDoor opens either shape); from_estate_dag + ALLEN'S DAG POLISH PAID (every card doors the template asset record that declared it — the ref was always at hand, always dropped; unknown refs door the view honestly); the standalone exporter collapsed to one idiom (svgOfDag reads act; the serpentine lane gained ⇩ svg). Proven live: /atlas serves act-v1 with resident doors + glow lists, the sheet unchanged; the estate suite-proven, its live receipt named to allen's next deployment. Suite 492. |
| `orreth_sim/actgraph.py` · `tests/test_actgraph.py` · `console_worker.py` · `orrethd/src/window.html` | Fable 5 (claude-fable-5) | 0067 sp4 — THE ACT ACCRETES: accrete_ask projects a live ask's whole picture from signed records alone (asker → 0066's choice node wearing its why/features → 0065's style → the answer's signals → every landed judgment); the picture GROWS as records land and degrades honestly when they leave; the /act door serves it on a click; the reader draws «the answer wears its thinking» beneath an ask's body. Proven live on the wire (chain whole, validate flawless, the why read aloud). Suite 495. THE DIVE CLOSES WHOLE — KCR-0004 PAID; four dialects, one language, one drawer, L1 never broken. |

## Fable 5 — 2026-09-09 (0068 sp1 — the guardrail grammar and the drawer)

| File | Author | Note |
|---|---|---|
| `orreth_sim/guardrails.py` · `tests/test_guardrails.py` · `console_worker.py` | Fable 5 (claude-fable-5) | 0068 sp1 (KCR-0003's payment begins) — THE GRAMMAR: match (pii · pci · pattern-by-ref, with direction) · action on the strength ladder (annotate<mask<quarantine<refuse) · a mandatory WHY; PII-masked/PCI-refused as genesis, planted once at the ask lane; the guardrails drawer (purpose by the razor, gate-checked rule by rule, teachings on siblings, empty legal-but-loud); THE LATTICE at publication (weakening/narrowing refuses with the teaching; absence never removes); THE 0071 SEAM PAID EARLY (the ask-cache keys on the live composed set's hash — a turned rail revalidates everything). Proven live incl. the full revalidation walk; the walk's find (the pull dropping guardrail-* → genesis re-planting beside the human's word) fixed in-hour. Suite 503. |
| `orrethd/src/main.rs` · `orrethd/src/pg.rs` · `orrethd/Cargo.toml` · `contracts/v0/tier-profile.schema.json` (L3, JB's rule-9 word) · `console_worker.py` | Fable 5 (claude-fable-5) | 0068 sp2 — THE RESOLVER WAKES: the dead crate called at last (compose_context = the parity-pinned fold + the guardrail pin, re-hashed); persisted to resolved_contexts (the durable referent); GET /context + POST /context/guardrails (a turned rail re-addresses the law in a breath); becky SIGNS the standards bundle (the plane never signs), bundle_ok verifies at push AND pull, a poisoned standard KILLS the boot, unsigned pulls confess; floor_flaw = the Rust monotone check (weaken/shorten refuses the whole boot); floors DECLARED in the sacred schema. Proven live incl. the pin re-address and three floors' durable contexts. Suites 503 + 24 + orrethd 12. |
| `orrethd/src/main.rs` · `console_worker.py` · SDK `client.py` · `smoke_orrethd.py` | Fable 5 (claude-fable-5) | 0068 sp3 — THE GATEWAY PIN: /model/authorize demands the ResolvedContext id (missing/stale = the one face; explicit off-switch confessed at boot); the grant returns the pin; every meter line carries it verbatim (6 worker bodies + the SDK) — «which guardrails governed this answer» is one SQL join; the worker's three thought lanes pin memo'd (self-busting on re-address); the SDK refetches once on stale and retries. Proven by smoke (no-law/stale/true-pin) + live (a consult under the demand; the join reading sonnet-5 under gr-4a7de4a1 with pre-worker floors confessing). orrethd 13 · 503 · 24. |
| `orreth_sim/rails.py` (new) · `tests/test_rails.py` (new) · `console_worker.py` · `model_plane.py` · SDK `rails.py` (new twin) + `chassis.py` + `test_parity.py` | Fable 5 (claude-fable-5) | 0068 sp4 — THE RAILS AND THE AUDIT: detectors as firmware (Luhn-checked PCI; PII roster; pattern-by-craft-ref confessing when unresolved); the ladder enforced exactly (annotate<mask<quarantine<refuse, refuse outranks); enforcement in the LiteLLM lane both directions (entering refusal spends nothing; leaving enforcement never un-meters); governed_thought + governed_voice (words AND grounded facts; refusal loud, never a silent fallback) + LiveGateway/PlaneClient + SDK GovernedThink(rails=); scribe-signed guardrail-audit either way carrying counts never content, derived_from = the law's head; vigil stages quarantine cards. Proven live: refuse/mask/quarantine/walk-back all walked; four outcomes on the log; audit body content-free through the governed door. 517 · 36 · Rust green. SDK bumped 0.4.0 for the close. |
| `guardrails.py` (scary_flaws) · `dials.py` (stamp-window-days) · `console_worker.py` · `window.html` · `askdoor.py` · `test_rails.py` | Fable 5 (claude-fable-5) | 0068 sp5 — THE STAMP: scary edits hold at the gate; the stamp card wears what lapses; approval mints the windowed consent record (cites the card) and lands the edit under it through the one door; the confession rides reply + envelope (pre-0068 envelope line retired) + glass banner; the ungated one-click walk-back (revoked sibling + restore); revoked/lapsed stops governing at the read. SAME MOTION, JB's inbox walk paid: objDecide's dropped port (dead buttons off :4500), the bell's ghost asks (patience + durable decline + self-withdrawing ghosts, proven on req-1909), fuel cards name joined agents, guardrail-review lifecycle, and the GATE_WHY 💡 explainer on every gate card. Lifecycle proven live end to end; 519 · 36. |
| `console_worker.py` (one line) | Fable 5 (claude-fable-5) | Wound found by JB in worker.log (2026-09-10): the 0063 dial migration left the feedback-routing loop referencing the deleted STUDIO_DARK_S constant — a NameError that silently killed each beat's whole 👎-routing pass whenever a classify leg aged past the dark window. Turned to dial_value("studio-dark") like its two sibling sites. 519 green; worker replanted clean. |
| `console_worker.py` · `window.html` | Fable 5 (claude-fable-5) | Two wounds JB's swarm-floor walk found (2026-09-10): (1) request ids are only unique PER FLOOR (req-N-epoch), and a whole-rig restart births identical ids across floors in the same second — the witness de-dupe keyed on id alone, so one floor's withdrawal swallowed its twins and the swarm's no-pulse card stood forever; now keyed (scope, id), the log names the floor, proven live (the stranded card withdrew itself; 0 staged witnesses across 26 floors). (2) The one-glass port family: ALL seven decide doors sent location.port — the SERVING floor — so after any floor-step a click resolved against the wrong queue and died silently; floorPort() now reads the VIEWED floor (API base) at every decide site. |
| `orreth_sim/basket.py` (new) · `tests/test_basket.py` (new) · `dials.py` · `artifacts.py` · `console_worker.py` · `window.html` | Fable 5 (claude-fable-5) | 0069 sp1 — THE BYTE LAW AND THE BASKET: pointer-path imports (mass content-addressed in the object store, signed 0039 §6 pointers with origins on the log — never base64 in bodies); zips open like folders, members remember their zip; the queue IS the log (signed job first, derived progress, interruption named); bars became dials (upload-inline-kb · import-max-mb); the Basket in the glass (picker under a declared root, one-faced traversal, running list with origins, one Import, log-read progress) + three :4562 doors. Proven live incl. the crash walk (killed mid-carry of 8; the log alone finished 8/8, zero duplicates). 519→529. |
| `orreth_sim/stores.py` (new) · `tests/test_stores.py` (new) · `basket.py` (import_bytes tail + store entries) · `dials.py` (store-freshness-min) · `console_worker.py` (svc_probe · store branch at the one door · on_store_onboard · the freshness eye · Basket store lane) · `window.html` (store sources + gate words) · `pyproject.toml` (boto3) | Fable 5 (claude-fable-5) | 0069 sp2 — THE STORE KIND AND THE FIRST WIRE: list-and-fetch citizens on the Farm's one lifecycle (manifest = declared ops; ambient creds; traversal law; rug-pull door; rest = free refusal); the two-keeper law in one flow (allen's charter record + charlotte's carried plant on ONE word); the freshness eye at a dial's cadence; the Basket browsing and importing THROUGH the metered door (bulk never in a response). Proven live: both stores onboarded→serving; the first cloud import (1.1 MB from S3, pointer citing store origin); freshness observed; the lifecycle walked. 529→537. |
| `orreth_sim/extract.py` (new) · `tests/test_extract.py` (new) · `basket.py` (line + rails + pay_parked) · `artifacts.py` (line + rails) · `console_worker.py` (rails threading · audits · extraction_line_beat) · `pyproject.toml` (pypdf) | Fable 5 (claude-fable-5) | 0069 sp3 — THE EXTRACTION LINE: PDF/Office/HTML deterministic and firmware-versioned (Office via stdlib zip+XML); failed parses named; eyes-needing formats park naming their eye; 0068's redaction-at-ingest wired (refuse = no knowledge minted, events never content, audits in the extraction lane); lineage everywhere; the parked lot swept and PAID (librarian-handled law). Proven live: Word extracted + card-doc refused in one job; the cloud PDF park paid whole (100k chars, chain to {store,key}); the fake PDF honestly unpayable. 537→545. |
| `orreth_sim/wiki.py` (new) · `tests/test_wiki.py` (new) · `stacks.py` (pointer_text_reader hook) · `console_worker.py` (wiki_beat · _WikiView · the railed hook · librarian's wiki grounding · pull filter's third strike) | Fable 5 (claude-fable-5) | 0069 sp4 — THE WIKI: grounded claims (evidence id = version; origin cited); origin/concept/catalog pages rebuilt as siblings; link whitelist; THE CASCADE READS THE SEALS (0026's death notices — never door-probes; the 0042 blindness law structural); flagged pages refuse current until a recorded rebuild NAMING the retraction; whole documents feed the Hierarchical tree via the railed hook — the walk's own wound (raw text around the ingest rail) paid in-hour, rows truncated, hash-drift law standing guard. Proven live incl. a real seal → cascade → flags → rebuilds. 545→553. |
| `stores.py` (database kind) · `meaning.py` (declared model) · `stacks.py` (one-truth standard + policy wears the model) · `mirror.py` (model-relative tau) · `farm.py` (resting twin) · `pg.rs` + `main.rs` (model column, both doors) · `console_worker.py` (db branches · _aim_embedding_standard · one-face polish) · tests | Fable 5 (claude-fable-5) | 0069 sp5 + CLOSE — THE DATABASE KIND (sqlite read-only double-locked through the same card/door; PostgreSQL/MySQL named growth) + THE STANDARD'S ONE TRUTH (multilingual genesis by L3; vectors wear their model; wrong-model rows are the sweep's work; the chunk policy wears the model — the migration structural). The resting twin paid. Proven live: db onboarded→serving, SELECT served, writes one-faced; the great re-embedding measurable (31,525 pre-standard, 64/beat). 553→558 · Rust 24. 0069 CLOSED WHOLE — sixth out-of-order close, VERSION holds 0.71. |
| `parlor.py` (make_thread_head · thread_turns · dial-fed caps · body-riding prior) · `dials.py` (thread-ask-chars · thread-reply-chars · thread-turns) · `console_worker.py` (_thread_context · the head mint · lineage on turns · /thread door) · `window.html` (thread rides both ways · 🧵) · `tests/test_thread.py` (new) | Fable 5 (claude-fable-5) | 0070 sp1 — THE THREAD: conversations become worldlines (server-minted head; every turn cites head + prior; replies SEE recent turns from the records under a dial; the DOM-only thread retired; the rebuild door). Proven live: the librarian remembered hempcrete across turns and named her scope honestly; chain-order rebuild; lineage receipts in pg. 558→562. |
| `profile.py` (parse_prefs · live_prefs · slice_text · typed make_claim) · `canon.py` (floored_tags) · `pg.rs` + `main.rs` (exclude_tags + evict_floored_embeddings) · `console_worker.py` (_profile_slice · kit injection · memo busts · per-floor embed clock) · `tests/test_profile_read.py` (new) | Fable 5 (claude-fable-5) | 0070 sp2 — THE PROFILE READ: the sovereign slice reaches the voice provenance-labeled; typed prefs (sovereignty-only, latest wins, language honored); THE FLOOR'S WALK-BACK — 134 profile vectors found in the meaning axis, the floor's tag list now Canon-derived and riding every worklist ask, the door evicting the old blindness (zero standing); the embed clock's shared-clock starvation cured per-floor. Proven live: a Spanish voiced reply carrying facts+preference+observations; withdrawal → English mid-thread. 562→566. |
| `orreth_sim/persona.py` (new) · `tests/test_persona.py` (new) · `speech.py` (nine blurbs, byte-mirrored) · `console_worker.py` (persona_text · the planter · the ⟦persona⟧ slot in firmware genesis + the live-shelf record · the gate branch · THE L2 CARVE-OUT · filter's 4th strike) · `window.html` (shelf blurbs) | Fable 5 (claude-fable-5) | 0070 sp3 — THE PERSONAS: 0046's promise collected under JB's razor word; honesty floor tighten-only; persona = PURPOSE in the prod gate's own code for ALL residents; blurbs retired to the shelf. Proven live: two unmistakable voices, a mid-life edit heard (stable-keeper → harbormaster → home), the floor riding every idiom, a shapeless edit refused. 566→570. |
| `orreth_sim/whenwords.py` (new) · `stacks.py` (parse_time speaks human) · `wiki.py` (claims_asof) · `parlor.py` (the belief lane) · `console_worker.py` (wire_belief_scrub · the window's when-words) · `tests/test_scrub.py` (new) | Fable 5 (claude-fable-5) | 0070 sp4 — THE SCRUB: deterministic when-words (ambiguity refused BY NAME); the temporal clause speaks human; events by the dial finally ON the answer path (the window frames itself to a heard day); beliefs by versions-as-time (claims_asof — believed THEN, retracted SINCE, records alone). Proven live in the glass's door three ways + the window's 🕰; controlled-clock suite for the replay flag. 570→574. |
| `orreth_sim/persons.py` (new) · `profile.py` (person strokes + stroke_visible) · `parlor.py` (register lane · person on turns) · `console_worker.py` (person_seat/custody · _person_ctx · on_person_join · person-scoped portrait/slice/forget/read · the person's thumb) · `window.html` («I am jb» · person on asks+thumbs · gate words) · `tests/test_persons.py` (new) | Fable 5 (claude-fable-5) | 0070 sp5 + CLOSE — THE PERSON: 0012's debt paid (humans as identities at becky's gate; names never reissued; acts signed under the person's own key; the floor seat the honest anonymous fallback; custody+registry must agree). Proven live end to end incl. the node verifying a human's own signature and custody surviving a replant. 574→578. 0070 CLOSED WHOLE — seventh out-of-order close, VERSION holds 0.71. |
| `extract.py` (pypdf logger quieted) · `console_worker.py` (_EXLINE_FAILED per-life memory) | Fable 5 (claude-fable-5) | JB's log find (2026-09-11): the unpayable park (the deliberately-fake demo site-plan.pdf) was retried EVERY 5-minute sweep forever, and each attempt let pypdf's own logger leak a raw «EOF marker not found» beside our named message. Now: one voice (ours — the ExtractionFailed already carries the fact in plain words) and one honest naming per worker life (a restart retries once — the file may have healed; eternity may not nag). Verified live: 1 named print, 0 raw leaks. |
| `capabilities/e-rag/genesis.py` (new) · `console_worker.py` (CAP_PANEL_KINDS +chat+sources · the canvas thread at on_ask: head mint, [choice,thread,prior] lineage, threaded asks skip the cache, thread in the envelope) · `window.html` (chat + sources arms rendered blind · rooms live without reports · chatSend/chatPoll/chatWait · srcLoad · capFresh's after-await guard · the composer survives every rebuild) · `tests/test_oneplace.py` (new) | Fable 5 (claude-fable-5) | 0072 sp1 — THE GRAMMAR GROWS: the rich rooms as kernel glass (capabilities declare, the kernel renders; no foreign code); E-RAG the first One Place tenant as PURE DATA; the chat canvas a worldline at the ask door. Proven live in the glass (two-turn walk, pg lineage receipt); four glass wounds caught by the walk and paid in-hour (rooms-without-reports · the ⟲ keep-watching door, proven twice · one wait-line in flight · the eaten half-typed ask cured at both ends). 578→581. |
| `window.html` (capOf exact · capUnder lineage) | Fable 5 (claude-fable-5) | JB's brain-view find (2026-09-11): E-RAG is the FIRST capability to declare an ECOSYSTEM as its floor, and capOf's descendant match (`startsWith(f+"/")`) stamped its chip + jade band on every variant floor beneath — eight «E-RAG · One Place» labels piling onto the sky. The cure separates LABEL from LINEAGE: capOf is exact (the standing chip lives at the world's own floor, once); capUnder keeps the descendant reach for what deserves it (jade memory-formation fire + the «purpose at work» ticks). Verified by screenshot: one chip per world. The residual label-proximity crowding in dense regions is a NAMED SEED for a collision-aware label pass — older than this dive, not paid here. |
| `orreth_sim/workspace.py` (new) · `tests/test_workspace.py` (new) · `console_worker.py` (CAP_PANEL_KINDS +workspace · /workspace door · wire_workspace · the draft lane: gate-checked at on_ask, head-swapped in wire_stacks_answer, hash+tag on the exchange, envelope confession, cache skipped) · `window.html` (the workspace arm: roster/canvas/inspector/eats · wsLoad/wsRun/wsSave/wsCompare/wsVersion · srcLoad reusable · ROOMS+workspace · composer survival covers ws-ask) · `capabilities/e-rag/genesis.py` (workspace declared) · `tests/test_oneplace.py` (rooms law grew) | Fable 5 (claude-fable-5) | 0072 sp2 — THE WORKSPACE ENGINE: one pane of glass for tuning the eleven, kernel-reusable (declared, rendered blind), every edit through the one-motion craft-edit gate — never a side door. Compare's alter-and-prove loop live end to end: draft held to the landed law (the gate teaches), head-swap on the disposable node (0043's proven trick), the exchange wearing draft {hash, knobs} + draft-config tag (pg receipt), «version it» landing the gated sibling with the comparison note, the roster re-serving tuned. 581→587. |
| `orreth_sim/workspace.py` (monitor_fold + _pct) · `tests/test_workspace.py` (2 operate laws) · `console_worker.py` (CAP_PANEL_KINDS +monitor+api-console · the fold + this floor's hands on the /workspace door · THE CHECKOUT-ERA UPGRADE LANE at the capability-* plant) · `window.html` (monitor + api-console arms · APIC rename after a parse-killing collision · focus survives re-renders even over an empty value) · `capabilities/e-rag/genesis.py` (five rooms) · `tests/test_oneplace.py` | Fable 5 (claude-fable-5) | 0072 sp3 — MONITORING + THE API CONSOLE: the operate room from the one truth (nearest-rank percentiles, recent-strip doors, raised hands — the boot-lap starvation measured where the human sees it: naive p50 1.3s/p95 17.5s) and the same ask door shown RAW (request · curl · envelope with the draft lane riding visibly). Three walk wounds paid in-hour: the plant-once manifest freeze (upgrade lane with lineage, human state preserved; missed-head partial NAMED — KCR-0002's box pays whole), the API identifier collision that killed the glass at parse, and the eaten-ask family's third member (focus over empty). 587→589. |
| `orreth_sim/package.py` (new) · `tests/test_package.py` (new, 6 laws) · `console_worker.py` (on_plant_package · _package_crew_body · discover_installed on the beat · the dispatcher branch · the honest head lane) · `window.html` (the plant-package card's teaching) | Fable 5 (claude-fable-5) | 0072 sp4 — THE PACKAGE, KCR-0002 PAID (the last unpaid KCR): orreth-package-v1 (bytes = identity · publisher signature self-certifying · crew containers-only, the checkout era's shell words refused · receiving vocabulary enforced); the plant-package human gate (tamper refused at the door live); the generalized discoverer (installed boxes re-verified every boot); the STRANGER WALK whole — packed outside the repo, installed through the gate, then the checkout folder REMOVED and the world lived from its box alone (five rooms, glass whole, an ask answered). One wound in-hour: an identical manifest is the SAME record on a write-once log — the head story now honest both ways. 589→595. |
| `orreth_sim/workspace.py` (showback_fold · quota_check) · `dials.py` (ask-quota-hourly, full jacket) · `tests/test_workspace.py` (2 laws) · `console_worker.py` (the person lane at on_ask · the teaching quota · envelope.authority · showback on the door) · `window.html` (showback table · latency-class select · person on the console · cxExport with the named floor + the empty-act confession) | Fable 5 (claude-fable-5) | 0072 sp5 — THE ENTERPRISE FIVE, each in its room: authority (jb's DID vs the shared-floor confession; person on the exchange body in pg) · the quota that TEACHES (turned to 2 live through the craft door; the refusal named limit, count, window, and dial; genesis restored) · latency classes honored ({fast} → honored:[latency]) · showback («the honest meter, not a bill»; 👤 jb beside the anonymous pool) · the compliance export (orreth-compliance-export-v1: asked + envelope + a 4-node act graph; the act door's floor named by the glass; the 0067-era pull-budget horizon confessed when empty). 595→597. |
| `spine/` (NEW: compose.yaml · pyproject.toml · orreth_spine/{envelope,rails,heartbeat}.py · tests/test_envelope.py · README.md) · `.github/workflows/ci.yml` (rearch/** push + spine job) · `docs/rearch/0005` (sp1 row) | Fable 5 (claude-fable-5) | REARCH P0 sp1 — THE NEW RIG BREATHES (first spoonful after the halt lifted, under canon 0001–0005): the spine home; orreth.transport/1 envelope v0 (canonical bytes = the contract · content hash · required fields refuse BY NAME · the authority chain in order — AG-7 attribution born in the first message); the dev rig (postgres 5433 · rabbitmq · kafka KRaft) with the ground writing heartbeat + outbox in ONE transaction from breath one; the heartbeat proof through all three rails, bytes exact — warm: ground 21 ms · invoke 16 ms · events 215 ms (vs the old ask p50 1,300 ms). 9 envelope laws; CI extended to the rearch line. |
| `spine/baseline/measure_m0.py` (new) · `docs/rearch/baselines/m0-old-world.md` (new) · `docs/rearch/playwright-agent-charter.md` (new) · `docs/rearch/experience-specs.md` (new) · `docs/rearch/baselines/before-walk-2026-09-16.md` (new) · `docs/rearch/0005` (sp2+sp3 rows) | Fable 5 (claude-fable-5) | REARCH P0 sp2+sp3 — THE OLD WORLD MEASURED + THE PLAYWRIGHT CHARTERED: M0 banked live (20 floors · universe poll 3.7 MB/fetch over 2,684 retained rows · ask path p50 120 s with 2/6 never answered and 373 MB read just to wait · idle ladder 289 MB/s at 10,100 floors · comparison contract vs locked SLOs — the spine invoke rail already 125× the best-case dispatch); the Playwright born (enforce-never-author · human path only · friction is a finding · honest waits) with 14 walkable specs from P1–P18; before-walk PARTIAL BY NAME (SPEC-ESC-01 FAIL confirmed — Escape left becky's drawer open — before the session spend limit killed the agent mid-walk; re-walk owed, nothing blocks). |
| `orreth_spine/outbox.py` (new) · `orreth_spine/inbox.py` (new) · `orreth_spine/envelope.py` (aggregate) · `tests/test_outbox_inbox.py` (new, 10 laws) · `tests/conftest.py` (new, throwaway-schema ground fixture) · `tests/test_envelope.py` (+1) · `.github/workflows/ci.yml` (postgres service, SPINE_REQUIRE_PG) · `docs/rearch/0005` (P1 sp1 row) | Fable 5 (claude-fable-5) | REARCH P1 sp1 — THE DURABILITY BOUNDARY (M1): the transactional outbox (one tx: domain + event; rolled-back state never emits; success never depends on the sink; budget refuses BY NAME) · the at-least-once relay (crash-before-publish retries; crash-after-publish duplicates HARMLESSLY) · the durable inbox (5 deliveries = 1 effect, duplicate meter confesses; failed effects roll back their footprint) · per-aggregate ordering (stale skipped; GAPS REFUSE TO GUESS, naming expected vs got). The whole SOL M1 fault schedule as deterministic tests — 20 green in 0.22 s; CI runs them against a real Postgres service, missing ground = FAIL never skip. Stop-condition honored: no distributed transaction anywhere. |
| `orreth_spine/sinks.py` (new) · `orreth_spine/projector.py` (new) · `tests/test_events_shadow.py` (new, 4 laws) · `.github/workflows/ci.yml` (Kafka service, SPINE_REQUIRE_KAFKA) · `docs/rearch/0005` (P1 sp2 row) | Fable 5 (claude-fable-5) | REARCH P1 sp2 — THE EVENTS SHADOW (M4-lite): KafkaSink (topic = envelope TYPE, schema families never per-identity; key = aggregate id — order exactly where it matters); the projector (offset commits ONLY after the db tx; poison PARKS visibly with reason + raw bytes and STOPS — an operator advances, never the code). Proven live on the real rail: project → replay-from-zero → burn-and-rebuild all equal the domain truth; wire duplicates fold to one effect; apply-then-die-before-offset redelivers harmlessly. Suite 24 green (42 s incl. live Kafka); CI now carries BOTH rails as real services — absence fails, never skips. |
| `orreth_spine/bridgefeed.py` (new) · `tests/test_bridgefeed.py` (new, 6 laws) · `docs/rearch/0005` (P1 sp3 row; PHASE 1 WHOLE) | Fable 5 (claude-fable-5) | REARCH P1 sp3 — THE BRIDGE FEED v0 (M6-lite): the one place the glass connects — SSE, pointer-only notices ({rev,kind,ref,message_id,at}; never bodies, never broker creds; the browser NEVER touches a broker); monotone revisions + bounded ring; gap repair rides SSE Last-Event-ID (laptop closes, facts land, reconnect misses NOTHING); beyond-the-ring answers resync HONESTLY; the gateway declares its topics (subscribe-before-first-fact race found + cured in-hour). Measured: COMMIT → GLASS NOTICE IN 124 ms (bar 1 s; the old world made the human poll 373 MB to learn the same thing). Suite 30 green. PHASE 1 THE SPINE IS WHOLE: ground → rail → projection → soft notice, every law on real services in CI. |
| `orreth_spine/resident.py` (new) · `orreth_spine/identity.py` (new) · `orreth_spine/dispatch.py` (new) · `orreth_spine/outbox.py` (add_row) · `orreth_spine/projector.py` (self-sufficient schemas) · `spine/policy/covenant-policy.v1.json` (new) · `spine/templates/echo-resident.v0.json` (new) · `tests/test_resident.py` (new, 6 laws) · `pyproject.toml` (+langgraph +pynacl) · `docs/rearch/0005` (P2 sp1 row) | Fable 5 (claude-fable-5) | REARCH P2 sp1 — THE BODY IS BORN (canon 0004 boot graph, real): versioned template or refused · the SAME self every life (persistent Ed25519; AG-2: respawn = same DID, lives counted) · covenant policy a versioned artifact, NO POLICY NO JOIN EVER (AG-3; the join wears version+hash, signed) · a real LangGraph mind proving the FULL-REPLY LAW (every word verbatim) · the road: one-tx ask+event → dispatcher (fact→command) → durable-inbox service with journey+reply rows AND events in ONE tx, ACK after commit · replayed road leaves the reply untouched · stale commands = terminal visible refusal, never a crashloop · the AUTHORITY CHAIN rides every hop (AG-7 seed live) · the feed shows received→journey→reply, completion last. Suite 36 green. |
| `orreth_spine/gateway.py` (new) · `orreth_spine/store.py` (new) · `orreth_spine/resident.py` (recall node + gateway think) · `spine/templates/librarian-resident.v0.json` (new) · `tests/test_mind.py` (new, 4 laws) · `pyproject.toml` (+anthropic) · `docs/rearch/0005` (P2 sp2 row) | Fable 5 (claude-fable-5) | REARCH P2 sp2 — THE MIND ARRIVES: the metered gateway lane (NO MIND THINKS OFF-METER — every thought lands did·model·tokens; the kernel never sees the prompt) · the OrrethStore v0 (memories content-hashed WITH their event in one tx; verbatim get; word-match search, honestly v0) · recall packs the worldline + matching memories; a mindless template NEVER touches the gateway (proven). THE LIBRARIAN THOUGHT FOR REAL: recalled the seeded fact, cited it BY MARKER, answered completely — 792 in/106 out on Haiku 4.5. Walk-wound in-hour: sentence-match search found nothing → word-based (and the honesty law had already held: she refused to invent the memory she could not find). Streaming deltas = NAMED DEFERRAL to P3 (pointer-only feed law; no glass yet). Suite 40 green. |
| `orreth_spine/tools.py` (new) · `orreth_spine/gateway.py` (think_acting + FakeActingGateway) · `orreth_spine/resident.py` (hold path · _confirm_ask · seq counter · confirm branch) · `orreth_spine/dispatch.py` (confirm_ask) · `templates/librarian-resident.v0.json` (tool capabilities) · `tests/test_soul.py` (new, 5 laws) · `docs/rearch/0005` (P2 sp3 row) | Fable 5 (claude-fable-5) | REARCH P2 sp3 — THE SOUL CHECKPOINT, PHASE 2 WHOLE: the governed tool door (declare or the door refuses WITH a teaching; the mind never sees undeclared tools; every call journaled) · the acting mind (real tool-use loop, every round metered, every act through the door) · THE L2 INTERLOCK REAL (consequential holds — "cancel is the default" — the act provably NEVER ran; a deliberate yes releases "Done, on your word"; cancel proven too). Wounds in-hour: the Exception.args collision (base Exception overwrites .args — held tool-args became a list) · the clarifying-question dodge (cured in the tool's own words: never ask where). THE LIVE WALK: «What's the temperature outside?» → «Right now it's 62.8°F outside where you are… A pleasant enough day, I'd say.» — weather journaled ✓, 860/43 metered. THE HALT'S SENTENCE IS A PASSING TEST. Suite 45 green. |
| `orreth_spine/glass.py` (new) · `spine/glass/index.html` (new — the Bridge v0) · `tests/test_glass.py` (new, 2 laws) · test-suite persistent-dispatcher cure (3 files) · `docs/rearch/0005` (P3 sp1 row) | Fable 5 (claude-fable-5) | REARCH P3 sp1 — THE GLASS EXISTS: the glass server (page · SSE feed · human doors: /ask, /ask/<id> w/ journey, /confirm where ABSENT approve = cancel) + BridgeRig (the whole dev Operating State in one object; `python -m orreth_spine.glass`). The Bridge v0 page: hull + amber strips + starfield window + ORRERY·BRAIN·ATLAS sill + OBJECTIVES floor + the One Chat OUT OF THE CEILING (journey soft-lines live · full reply in place · interlock in-chat, Cancel-default holds focus, Enter can NEVER confirm · [Esc] returns the bridge — SPEC-ESC-01 cured). Notices pointer-only; truth through doors. Proven over HTTP alone incl. cancel-never-ran. THREE suite wounds in-hour, each a production lesson: fresh dispatcher groups replayed ALL history (the tests HAD the growing-corpus disease — cured with the production pattern, one persistent group) · the ring overflowed honestly at 1024 (listen like a client) · the grown graph moved the reply past a hard count (collect until the reply). Suite 47 green, twice stable. |
| `orreth_spine/glass.py` (/asks · /residents · fan-out /ask · two-seat BridgeRig) · `orreth_spine/dispatch.py` (targeted routing · scoped dispatcher · routed confirm) · `orreth_spine/resident.py` (per-resident queues · serve_queue/serve_key ns) · `orreth_spine/envelope.py` (scope()) · `spine/glass/index.html` (the band · the crew rail · fan-out + summarize-on-demand) · `tests/test_band_fanout.py` (new) · conftest (session world) · `docs/rearch/0005` (P3 sp2 row) | Fable 5 (claude-fable-5) | REARCH P3 sp2 — THE BAND, THE EDGES, THE FAN-OUT: the OBJECTIVES hatch opens (P13 felt — every row a door, replies+journey in place, live on notices) · the crew rail (selection IS the scope of applied inference; targeted routing ends the catch-all lottery) · fan-out real (one ask per named resident, two lenses two selves, summarize-together only when all land — synthesis is the human's CHOICE). TWO structural wounds cured at the right layer, both found by LIVE COLLISION: the running Bridge answered a test's ask (with yesterday's weather in recall!) → queues wear a NAMESPACE and every fact wears its WORLD (dispatchers dispatch only their own world — universe isolation arriving early, proven by accident) · an old-code process is an OLD WORLD no new law can bind (the deployment lesson, named for the cells era). Suite 49 green. |
| `orreth_spine/gateway.py` (on_delta streaming, DDL lock) · `orreth_spine/resident.py` (on_delta → feed · settled-status guard · schema pre-flag) · `orreth_spine/bridgefeed.py` (publish_delta · SSE `delta` frames) · `orreth_spine/projector.py` (run_forever standing consumer · skip-before-inbox) · `orreth_spine/dispatch.py` (run_dispatcher) · `orreth_spine/glass.py` (standing dispatcher · join retries · bench sweep · wait_ready · stop joins threads) · `orreth_spine/outbox.py` (once-per-connection guard) · inbox/store/tools (DDL advisory lock) · `spine/glass/index.html` (…thinking aloud → the door's truth) · `tests/test_stream.py` (new) · conftest (module-scoped rig) · 5 test files (fresh-group dispatch, rig aiming) · `docs/rearch/0005` (P3 sp3 row) | Fable 5 (claude-fable-5) · Fable 5.1 (claude-fable-5-1) — the phantom diagnosis; Opus 5 (claude-opus-5[1m]) read only during a safeguard swap, no edits | REARCH P3 sp3 — THE WORDS FORM LIVE: deltas stream gateway → resident → glass feed → SSE `delta` to connected clients ONLY (never ring/revision/outbox/topic — structurally); the chat sketches under "…thinking aloud", the door's truth replaces it; concatenation == reply; the door still serves the whole. CI wounds cured at the layer: DDL race on a fresh ground (advisory lock, once per connection) · dispatcher group ghost-join (a STANDING consumer, one membership per life; world skip BEFORE the inbox footprint) · settled asks refuse re-serve. THE PHANTOM (hours lost, Fable safeguards tripped 5×): a Bridge relit 08:14 on pre-skip code marked every world's facts applied under the shared `glass-dispatcher` name on the shared ground → every test dispatcher read `stale` — the sp2 deployment lesson bitten twice; found by measuring one fresh rig hop by hop (Kafka append ts vs occurred_at, apply_event's verdict, thread stacks), not by patching. Second wound: a session-scoped test rig is a competing consumer → module scope, stop() joins. Suite 50 green, twice-run stable. |
| `docs/rearch/baselines/after-walk-2026-09-17.md` (new — the Playwright's record, 2 re-walks) · `baselines/after-walk-2026-09-17/` (18 screenshots) · `docs/rearch/experience-specs.md` (Bridge v0 column · after-walk record) · `spine/glass/index.html` (chat yields to the band · labelled synthesis · crew-naming prompt) · `orreth_spine/glass.py` (BridgeRig `home` · main seats persistent selves · ask_view exposes target) · `tests/test_glass.py` (rig-layer same-self law) · `tests/conftest.py` (pg autocommit · explicit ephemeral rig · readiness assert names the stalled event) · `docs/rearch/0005` (P3 sp4 row) | Fable 5.1 (claude-fable-5-1) — the builder; a Fable 5.1 Playwright subagent walked (record + evidence only) | REARCH P3 sp4 — THE PLAYWRIGHT WALKS THE SPECS: the AFTER-WALK of all 14 specs on the human path only (real Chromium via the Playwright MCP; no door, no ground, no source; honest waits 4–12 s; 18 bar-moment screenshots). THE WOUND RULE, WORKING: three wound-grade findings → ruled wounds → cured → re-walked green same day: (1) the band's doors shut behind the open chat (CSS: the chat yields) · (2) "summarize together" misattributed the echo's words and pasted the machine prompt as the human's (labelled synthesis + crew-naming prompt) · (3) every reply labelled "orreth" — root: the Bridge minted EPHEMERAL selves every life, covenant rule 1 drift (cured: seeds under ~/.orreth/agents, proven live lives=2 same DIDs; rig-layer law; reply falls back to the routed target). Suite wound: psycopg3's implicit transaction on the session `pg` connection pinned the DDL advisory lock → every later rig's dispatcher queued (autocommit). On the road, not footnoted: the ground carries no world scope (band/roster show every world — sp5 first) · JOURNEY-01 lacks scope+time · SCOPE/SCHED/LENS/CLEAR not built (expected) · fan-out crew unaware of each other · raw markdown · no download. Before-walk remainder owed (old rig, JB's spend call). Suite 51 green. |
| `orreth_spine/resident.py` (scope on spine_asks + spine_joins · join wears scope) · `orreth_spine/dispatch.py` (ask wears scope) · `orreth_spine/projector.py` (run_once: skipped facts free against the cap) · `tests/test_events_shadow.py` (cap law) · `orreth_spine/glass.py` (ask_view/asks_view/residents_view serve ONLY their world; ask_view exposes scope + times) · `spine/glass/index.html` (JOURNEY-01 line: who · scope · completed · elapsed; band rows carry a time) · `tests/test_glass.py` (+2 laws) · `docs/rearch/experience-specs.md` (JOURNEY-01 PASS · re-walk #3) · `baselines/after-walk-2026-09-17.md` (re-walk #3, 2 screenshots) · `docs/rearch/0005` (P3 sp5 row) | Fable 5.1 (claude-fable-5-1); the Fable 5.1 Playwright subagent re-walked | REARCH P3 sp5 — THE GROUND WEARS ITS WORLD: every ask and join carries its SPINE_SCOPE at the write; a glass's doors serve only their own world (another world's ask = "no such ask", one face) — the isolation law at the ground, not only on rails and benches. JOURNEY-01 PASSES (who · scope · completed · elapsed under each reply). Proven live: the Playwright re-walked LIFE-01 + JOURNEY-01 WHILE the full suite ran on the same ground — not one stranger's row. Dev-ground operator's act, once: 239 pre-column rows got their world from their own ask.received events (51 real dev asks back in the band, every test row gone). SUITE WOUNDS: the growing-corpus disease a second time — run_once counted skipped other-world facts against its 500 cap; at 539 facts on the topic every fresh-group test dispatcher stalled short of its own fact (cured: skips are free; pinned by a law in test_events_shadow) · the new isolation law polluted a later test (now files in its own world) · a background suite's exit code was the pipe's, not pytest's — read the summary line, never the pipe. Suite 53 green. |
| `spine/glass/index.html` (the clock assist: readWindow/readMoment · the TIME strip + day-ticked axis · the window rides the ask · journey line names it · returns to now after a send) · `orreth_spine/dispatch.py` (submit_ask window → payload + `time_window`) · `orreth_spine/resident.py` (time_window column) · `orreth_spine/glass.py` (door validates window · ask_view exposes it) · `tests/test_glass.py` (window law) · `docs/rearch/experience-specs.md` (SCOPE-01 PASS · re-walk #4) · `baselines/after-walk-2026-09-17.md` (re-walk #4 + re-check) · `docs/rearch/0005` (P3 sp6 row) | Fable 5.1 (claude-fable-5-1); the Fable 5.1 Playwright subagent walked | REARCH P3 sp6 — THE SCOPE EDGES: the chat's clock assist — typed words set the time scope as intent forms (P6), no click, no send; both edges named, a ticked axis between; the window RIDES THE ASK (payload + ground) and its journey line names it — scope, never eye candy (P8); after a send the clock returns to now. SPEC-SCOPE-01 PASSES on the human path (re-walk #4). Found the loud way: `window` is a reserved word in SQL (→ `time_window`). Left edge (universe/eco/field multi-select) owed to P4 — nothing to select until a second world exists. Suite 54 green. PHASE 3 THE BRIDGE v0 IS WHOLE (JB's lock 2026-09-17). |
| `docs/rearch/experience-capture.md` (Session 3, block 9) · `docs/rearch/0001-experience-charter.md` (workspaces · focus/includes/sessions section · P19–P21) · `docs/rearch/0003-memory-architecture.md` (sessions as worldlines · MEM-7) · `docs/rearch/0004-agent-architecture.md` (the third kind: firmware agents · the draft shelf · skill allocation · AG-8/AG-9) · `docs/rearch/0005-v1-scope-and-build-plan.md` (P4 re-sliced) · `docs/rearch/0006-markers-dive.md` (new SEED) | Fable 5.1 (claude-fable-5-1) | REARCH CANON — BLOCK 9, THE FEEL (JB's narration 2026-09-18, vetted and locked the same day): the chat follows the FOCUS (a pull pushes, Esc pops and restores the bridge's scope — JB's scope-drop line vetoed in the vet); includes are FIRMWARE AGENTS — the third kind: same body of laws, no persona, named by function, human- and kernel-callable, ONE workspace body with per-pull BINDINGS, the grader scribe-class, skills allocated by need from the versioned Orreth Skill; SESSIONS roll, never spill (the human's worldlines: roll · list · load; an agent reads that session only); eager for the next helpful step, always as an offer; the DRAFT SHELF (unversioned never enters the brain; the cut is the gate). P4 re-sliced: focus+sessions → includes → Crew workspace → Monitoring + its agent. Markers: a dive doc seeded, designed before P5. |
| `orreth_spine/resident.py` (session column + spine_sessions · recall bound to the ask's session, labeled) · `orreth_spine/dispatch.py` (submit_ask session) · `orreth_spine/glass.py` (open_session · sessions_view · session_view · GET/POST /sessions · GET /session/<id> · /ask carries session · ask_view session) · `spine/glass/index.html` (the focus stack: pushFocus/popAllFocus, Esc pops + restores · sessions by link and by words · the resizable chat · render-once · reload continues the session) · `tests/test_glass.py` (+3 laws) · `docs/rearch/experience-specs.md` (SPEC-FOCUS-01 · SPEC-SESSION-01 · walk #2) · `baselines/after-walk-2026-09-17.md` (walk #2 + re-walks, 9 screenshots) · `docs/rearch/0005` (P4 sp1 row) | Fable 5.1 (claude-fable-5-1); the Fable 5.1 Playwright subagent walked | REARCH P4 sp1 — THE CHAT FOLLOWS THE FOCUS + SESSIONS v0: an opened ask pushes a focus (the chat opens itself, re-dresses to the ask's resident and window), one Esc pops every focus, restores the bridge's scope EXACTLY, bare bridge (P19 and SPEC-ESC-01 agree). Sessions (P20): a session on every ask, three doors (roll · list · load), typed words equal the links, a loaded session renders whole; MEM-7's seed — a resident's recall reads THIS session's results only, by any resident, labeled. The chat is resizable (upper-right origin locked). THE WALK: FOCUS-01 PASS; SESSION-01 FAIL → PASS — a wound: the door never decoded the URL-encoded person ('none yet' for everyone; the Python views were tested, the handler's parsing was not — an HTTP-level law now pins it). Frictions cured alongside: clock under focus · calendar-day spans · render-once · visible reload · replayed journey lines carry the world. Suite 57 green. |
| `spine/templates/firmware-{planner,critic,grader}.v0.json` (new) · `orreth_spine/resident.py` (kind/function/charge · join kind · recall collects the read set, grader scribe-class · think: firmware system, refusal before thinking · the chain names the read) · `orreth_spine/glass.py` (crew door tells kinds · the rig seats the includes) · `spine/glass/index.html` (INCLUDES strip · include words · send target override · rail shows residents only · journey line names the read · plain-words bubbles · roll/load pop the focus · band header closes) · `tests/test_includes.py` (new, 3 laws) · `docs/rearch/experience-specs.md` (SPEC-INCLUDE-01 · walk #3) · `baselines/after-walk-2026-09-17.md` (walk #3, 2 screenshots) · `docs/rearch/0005` (P4 sp2 row) · `docs/rearch/0004` (open item resolved) | Fable 5.1 (claude-fable-5-1); the Fable 5.1 Playwright subagent walked (its last before the spend wall) | REARCH P4 sp2 — THE INCLUDES EDGE: the third kind is real — planner · critic · grader as firmware bodies (named by function, a charge not a persona, the same laws); an include = an ask targeted at a firmware body in the session, so its input is the session's labeled results; AG-8 by construction (recall collects the DIDs read; the chain lands H → residents read → firmware; a journey step names them); the grader scribe-class (own words never material; refuses before thinking with nothing else in view). Walk #3: planner PASS · grader PASS · journey-names-the-read FAIL in the chat (note emitted, line never showed it) → cured, re-walk owed. |
| `spine/templates/workspace-firmware.v0.json` (new) · `spine/bindings/crew.v0.json` (new — the binding schema v0) · `orreth_spine/resident.py` (Resident(binding=…) · join records declared capabilities · workspace recall packs the crew) · `orreth_spine/glass.py` (crew_view: both sides, kernel duties immutable · GET /crew · the rig seats the crew agent) · `spine/glass/index.html` (the Crew pull: left-wall tab, cards, 🔒 duties, assist soft links · workspace focus · "open the crew" by words) · `tests/test_crew.py` (new, 3 laws) · `docs/rearch/experience-specs.md` (SPEC-WORKSPACE-01 · SCHED-01 partial) · `docs/rearch/0005` (P4 sp3 row) | Fable 5.1 (claude-fable-5-1) — walk OWED (Playwright spend wall) | REARCH P4 sp3 — THE CREW PULL, THE FIRST WORKSPACE: one workspace body + a binding per pull (v0: bindings/crew.v0.json names the seat, prompt, skills; applied at birth); the Crew pull with one card per body and BOTH SIDES (asks served; kernel-required duties, visible never editable; human/role none until the scheduler — SCHED-01 PARTIAL); the chat follows the workspace (P19: FOCUS · the Crew workspace · its agent), the agent answers from the cards, manual vs assist felt (chat closed = cards by hand; a card's soft link reopens the chat to its agent). Walk owed to the single debug session per JB's cadence call. Suite 63 green. |
| `orreth_spine/presence.py` (new — leases, M2) · `orreth_spine/monitor.py` (new — snapshot + watches) · `orreth_spine/harness.py` (new — A/B v0, the failed-run fact) · `spine/golden/librarian.v0.json` (new) · `spine/bindings/monitor.v0.json` (new) · `orreth_spine/tools.py` (add-watch, consequential; the door passes the ground) · `orreth_spine/resident.py` (binding capabilities · monitor recall) · `orreth_spine/glass.py` (GET /monitor · POST /harness/run · cards carry alive · the rig renews leases and seats the monitor agent · the feed carries harness failures) · `spine/glass/index.html` (the Monitoring pull and its views · the harness soft notice in the chat · "open the monitor") · `tests/test_monitor.py` (new, 4 laws) · `docs/rearch/experience-specs.md` (SPEC-MONITOR-01) · `docs/rearch/0004` (AG-6 status) · `docs/rearch/0005` (P4 sp4 row) | Fable 5.1 (claude-fable-5-1) — walk OWED (Playwright spend wall) | REARCH P4 sp4 — MONITORING, THE HARNESS v0, PRESENCE LEASES: leases (M2) — alive while serving, dormant in seconds, never deleted; the Monitoring pull = the Operating State live through one door (rails · benches · topic depth · bodies on their leases · asks · watches · the last harness run); its agent PROPOSES watches through the interlock (the MITL loop in miniature: add-watch holds for the yes, lands, judged green/red); the harness v0 runs golden cases through a body's graph, records every run, and a failing run is a fact on the rail the chat shows as a soft notice. The scheduler is now the named gap (AG-4 CRUD · AG-6 scheduled run). Suite 67 green. |
| `orreth_spine/scheduler.py` (new — the organ: three kinds, tick, rest, the card's side B) · `orreth_spine/resident.py` (join registers the template's role schedules) · `spine/templates/librarian-resident.v0.json` (a declared role schedule) · `orreth_spine/glass.py` (GET /schedules/<runner> · POST /schedules · POST /schedules/rest with the kernel's 403 · the rig's schedule loop declares kernel duties and beats · EVERY long-lived rig connection autocommit) · `spine/glass/index.html` (the card's side B: kernel 🔒 · role · human with beats, rest it, a schedule form) · `tests/test_scheduler.py` (new, 4 laws) · `docs/rearch/experience-specs.md` (SCHED-01 built) · `docs/rearch/0004` (scheduler decided) · `docs/rearch/0005` (P4 sp5 row) | Fable 5.1 (claude-fable-5-1) — walk OWED (Playwright spend wall) | REARCH P4 sp5 — THE SCHEDULER, a kernel organ: human (CRUD through the door) · role (template-declared, registered at join) · kernel (rig-declared, immutable, one plain face); occurrences are asks to the runner on the rail (the band shows them with their journey), kernel ones act and are recorded; rest is recorded, never deleted (rule 11); every schedule in its runner's card (P16). AG-4 BUILT; AG-6's scheduled half BUILT (the harness every 30 min). PRODUCTION WOUND found by a hang: the schedule loop's bare SELECT opened psycopg3's implicit transaction and the next ensure_schema held the DDL lock forever — cured: every long-lived rig connection autocommit (the fixture lesson, now in code). Suite 71 green. |
| `baselines/after-walk-2026-09-17.md` (walk #4, 17 screenshots) · `docs/rearch/experience-specs.md` (INCLUDE-01 · WORKSPACE-01 · SCHED-01 · MONITOR-01 cells; walk #4) · `spine/glass/index.html` (pulls follow the feed + the monitor breathes · a confirm refreshes every lens · scroll kept on refresh · a close inside the chat · plain-words italics) · `docs/rearch/0005` (the walk session) | Fable 5.1 (claude-fable-5-1); a fresh Fable 5.1 Playwright subagent walked | REARCH P4 — THE WALK SESSION (walk #4): INCLUDE-01 ×3, WORKSPACE-01, SCHED-01 green; MONITOR-01 one WOUND — the open pull went stale (loaded once; a confirmed watch read 'none yet'; leases aged past 'until' still 'alive') — P17 felt failing; cured same hour (pulls follow the feed; the monitor breathes; confirm refreshes; scroll kept); re-walk owed — the one thing between P4 and WHOLE. |
| `orreth_spine/tools.py` (the acquire tool; the door carries the resident's name) · `orreth_spine/store.py` (memories wear their world · within by timeframe) · `orreth_spine/resident.py` (recall reads the ask's window: the human's asks across sessions + words acquired in it) · `orreth_spine/glass.py` (recall_view · GET /recall by ref/ask/session/window, 400 on a bad window) · `spine/templates/librarian-resident.v0.json` (tools:acquire) · `tests/test_recall.py` (new, 3 laws) · `docs/rearch/experience-specs.md` (FULL-01 · SCOPE-01) · `docs/rearch/0005` (Phase 5 opened; sp1 row) | Fable 5.1 (claude-fable-5-1) — walk OWED | REARCH P5 sp1 — MEM-1 TOTAL RECALL + THE WINDOW READS: acquire through the door (every word exact, content-hashed, with its event, in the resident's namespace; memories wear their world); recall by ref · ask · session · timeframe, byte-exact, one door; the ask's window is a lens over all the human's worldlines in this world — recall packs the asks in it (labeled, timed) and the words acquired in it; nothing outside comes along. Suite 74 green. |
| `orreth_spine/store.py` (rewritten: lineage · validity intervals · get/search as-of · history · the tsvector projection with GIN · in-place migration) · `orreth_spine/resident.py` (recall reads versions as of the window's end) · `orreth_spine/glass.py` (recall by ref: now · then · history) · `tests/test_understanding.py` (new, 4 laws) · `docs/rearch/0003` (projection v0 status) · `docs/rearch/0005` (P5 sp2 row) | Fable 5.1 (claude-fable-5-1) | REARCH P5 sp2 — UNDERSTANDING v0 (MEM-4): a memory never overwrites — a sibling supersedes, each with a validity interval; true-now and true-then both answer (get/search `at`, history, the door); in the chat a window reads memories as of its end. The projection v0: full-text, stemmed, OR-shaped over the ask's words, ranked, GIN-indexed, wearing `tsvector:english` per row; stopwords alone find nothing, a partial token falls back to the word-match. Older grounds migrate in place. Suite 78 green. |
| `orreth_spine/digest.py` (new — compose · build (idempotent / sibling) · of_session · for_person) · `orreth_spine/glass.py` (the roll carries the archived session · GET /digest/<session> · POST /digest · sessions carry the short version) · `orreth_spine/resident.py` (recall packs the digests FIRST) · `spine/glass/index.html` (the roll names the archived session; the list shows the short version) · `tests/test_digest.py` (new, 3 laws) · `docs/rearch/0003` (Digest status) · `docs/rearch/0005` (P5 sp3 row) | Fable 5.1 (claude-fable-5-1) | REARCH P5 sp3 — THE DIGEST (MEM-3): a session's roll is an episode boundary → the archived session's digest, extractive and deterministic (byte-identical on rebuild; a superseding sibling when the episode grew; each with its event), citing every source (asks, memories acquired in the span: this opening → the next); the pack reads the short version FIRST (digests of the human's earlier sessions, named); each source opens verbatim through the recall door; the sessions list shows the short version. Suite 81 green. |
| `orreth_spine/resident.py` (state on the serve · the checkpointed graph on its own connection · prepare() · resume at the pending hop) · `orreth_spine/store.py` (state on every read · purge + tombstone · the landed index) · `orreth_spine/digest.py` (state · rebuild_citing) · `orreth_spine/dispatch.py` (an ask inherits its session's state) · `orreth_spine/glass.py` (opt-out sessions · state in the list) · `orreth_spine/tools.py` (purge-memory, consequential) · `spine/templates/librarian-resident.v0.json` (tools:purge-memory) · `pyproject.toml` (+langgraph-checkpoint-postgres) · `tests/test_resume.py` · `tests/test_purge.py` · `tests/test_growth.py` (new, 4 laws) · `docs/rearch/0003` (MEM status) · `docs/rearch/0005` (P5 sp4 row; P5 built) | Fable 5.1 (claude-fable-5-1) | REARCH P5 sp4 — RESUME · QUARANTINE & PURGE · GROWTH: MEM-2 (the graph checkpointed after every hop on its own connection; a life that dies mid-think resumes at think, one thought spent, the journey says so) · MEM-5 (Opt Out is a state on sessions → asks → memories → digests; every read stays in its state; a governed purge through the interlock removes every version, rebuilds citing digests as siblings, leaves a tombstone of hashes) · MEM-6 (1k → 10k, search/within < 100 ms and flat). PHASE 5 BUILT; whole on its first walk. Suite 85 green. |
| `docs/rearch/0006-markers-dive.md` (DESIGNED) · `docs/rearch/0002-transport-architecture.md` (the marker rides every envelope) · `docs/rearch/0003-memory-architecture.md` (the lineage projection) · `docs/rearch/0004-agent-architecture.md` (every body marks, and acts on marks) | Fable 5.1 (claude-fable-5-1) | REARCH CANON — THE MARKERS DIVE (JB's locks 2026-09-19): markers are an OPEN, GOVERNED vocabulary of WHY — `marker {kind, id, parent, by}` on every envelope; a root mints, a serving fact carries, a new beginning mints a child; kinds DECLARED into a registry (kind · group · description · by) before use, the kernel seeding objective · intention · thought · action · observation; any body SETS markers on whatever it executes through the `mark` door, its policy saying when (every body: 'if you observe an improvement, mark it'); the INTEREST LAW: a marker set is a fact on the rail and the kernel dispatches an ask to every body that declared interest in the kind, the marker as parent — setting AND using markers drives automation; the chain says who, the marker says why. MK-1 revised. markers sp1 opens. |
| `orreth_spine/markers.py` (new — registry · seed per world · mint/insert · set_marker + the fact · interested/dispatch_interests · tree/ancestry/stream) · `orreth_spine/envelope.py` (marker field) · `orreth_spine/dispatch.py` (asks mint objective/thought, parent_marker) · `orreth_spine/scheduler.py` (intentions; occurrences under them) · `orreth_spine/harness.py` (a run is an observation) · `orreth_spine/tools.py` (the door carries the marker; actions marked; the mark tool) · `orreth_spine/resident.py` (the serve reads its marker; journey + reply carry it; the policy line; joins record interests) · `orreth_spine/glass.py` (GET /markers · /markers/kinds · POST /markers/kinds · POST /mark; the feed carries marker.set) · `spine/glass/index.html` (the MARKERS view: toggles per group/kind, the stream, every row a why; "mark this as …") · `spine/templates/*` (librarian marks; critic interested) · `tests/test_markers.py` (new, 3 laws) · `docs/rearch/0005` (markers sp1) · `docs/rearch/0006` (built) · `docs/rearch/experience-specs.md` (SPEC-MARK-01) | Fable 5.1 (claude-fable-5-1) — walk OWED | REARCH MARKERS sp1 — THE OPEN VOCABULARY OF WHY: a per-world registry (declare before use; the kernel seeds the structural five + improvement); the write path mints structural markers WITH the fact; every envelope carries {kind, id, parent, by}; the `mark` door + the policy line; THE INTEREST LAW (a marker set is a fact on the rail; interested bodies are asked to act, the marker as parent); doors for the tree, the ancestry, the stream; the MARKERS view with toggles. MK-1 proven: librarian marks an improvement → the critic acts under it → thought ← improvement ← objective. Suite 88 green. |
| `baselines/after-walk-2026-09-17.md` (walk #5, 5 screenshots) · `orreth_spine/resident.py` (the window's record FIRST under a header; the window law line) · `orreth_spine/markers.py` (interest asks land in the marked ask's session; the why in words) · `orreth_spine/glass.py` (a listed session gets its digest from the log; doors carry the words) · `spine/glass/index.html` (the chat shows the kernel's asks in its session; marker authors named; the why in words) · `tests/test_recall.py` · `tests/test_markers.py` (laws tightened) · `docs/rearch/experience-specs.md` (five cells; walk #5) · `docs/rearch/0005` (P4 WHOLE; P5 status) | Fable 5.1 (claude-fable-5-1); the Fable 5.1 Playwright subagent walked | REARCH — WALK #5 (lean): MONITOR-01's cure PASS → PHASE 4 CLOSES WHOLE; FULL-01 acquire PASS; two WOUNDS cured same hour — SCOPE-01 (the window's record packed deep and unlabeled; the mind denied it → first, under a header, named by law) · MARK-01's fourth bar (the critic's act outside the chat's session → in it, and the chat shows the kernel's asks); SESSION-01's short version not exercised (sessions predating the Digest now get theirs from the log). P5 whole on the next walk. |
| `docs/rearch/0007-intent-dive.md` (new, DESIGNED) · `docs/rearch/experience-capture.md` (Session 4, block 11 + the vet + the locks) · `docs/rearch/0001` (P22–P25; the Analyzer verdict resolves the floor hatch) · `docs/rearch/0004` (the firmware-rails table, the intent loop, the intention record) · `docs/rearch/0002` · `0005` (the intent line before P6) · `0006` (`intention` widened; MARKERS view → the Analyzer) · `docs/rearch/experience-specs.md` (SPEC-INTENT-01 · SPEC-ANALYZE-01 planned) | Fable 5.1 (claude-fable-5-1) | REARCH CANON — BLOCK 11 "INTENT" (JB's locks 2026-09-19): human INTENTION above Objective (the registry's `intention` widened to any standing purpose; a schedule its smallest form); the INTENT LOOP as the fifth firmware-rail (tick or an interesting marker → the planner under the intention → an objective to the crew, parent = the intention); THE ASK WEARS ITS KIND (P23 — no chat modes: a chip before send, the marker minted with the ask, the interest law re-dresses the chat — Fable's push-back, ratified); STATE IS A PROJECTION OF APPLIED INTENT (P24); THE ANALYZER (P25 — Objectives is NOT a pull, JB's correction accepted; the hatch groups by origin from the ground, MARKERS moves in); five Infinite Horizon Intentions (business · security · resiliency · compliance · cost), Resiliency first. Honest boundary held: not single-binary, not Rust, self-improving under human cut. intent sp1 opens before P6. |
| `orreth_spine/intent.py` (new — the record · read_words · declare/declared/stop · the loop: watch transitions → watch-red → the planner under the observation → the objective under the intention · cadence) · `orreth_spine/markers.py` (ROOT column + backfill · `watch-red` seeded · interests reach intentions · `origins`/`origin`/with_words in words) · `orreth_spine/dispatch.py` (`kind` on the ask) · `orreth_spine/monitor.py` (watches remember last_ok) · `orreth_spine/glass.py` (ask_view carries marker + origin · GET /analyzer · GET/POST /intentions · POST /intentions/stop · POST /ask takes kind + parent, declares an intention · the rig's `_intent_loop`: Resiliency at boot, a turn every 3 s) · `spine/glass/index.html` (the ANALYZER hatch: intentions with stop · objectives · an intention's tree · the MARKERS view moved in · the KIND chip · "follow up on this by …" · the kernel's word in the chat) · `orreth_spine/ground.py` (new — every ground ensured at a connection's birth) · `orreth_spine/resident.py` (serve_once ensures all grounds first) · `tests/test_intent.py` (new, 5 laws) · `docs/rearch/0005` (intent sp1 row) · `docs/rearch/0007` (built; the stop line) · `docs/rearch/experience-specs.md` (INTENT-01 · ANALYZE-01 built; MARK-01's view moved) | Fable 5.1 (claude-fable-5-1) — walk OWED | REARCH INTENT sp1 — THE ANALYZER, THE KIND OF AN ASK, THE FIFTH RAIL: an intention is a record with a root marker and its own session; the intent loop turns on a red watch (observed under every intention that cares, a fact on the rail) or a cadence — the planner asked under the observation, its reply filed as the objective to the runner with the intention as parent; Resiliency stands at boot; the stop is recorded (rule 11 — the kernel's too; JB may veto). THE ASK WEARS ITS KIND (P23): prefix > chip > the words' read; no modes. THE ANALYZER (P25): origins from a ROOT column — at 10k markers ~1 ms, measured (IH-3). WOUND at the relight, cured: the markers DDL inside one serve's transaction held an exclusive lock for the length of the serve and wedged every loop and door — `ground.ensure_all` at every connection's birth, doors autocommit, a law pins it. Suite 93 green. |
| `baselines/after-walk-2026-09-17.md` (walk #6, 5 screenshots) · `docs/rearch/experience-specs.md` (five cells) · `spine/glass/index.html` (kernel-born objectives render like any ask, their replies land in the chat · WHOSE word · the open Analyzer row survives a repaint · a twig is a door across its row, the reply whole · the empty composer reads THOUGHT) · `orreth_spine/glass.py` (an ask's origin carries `by`) · `orreth_spine/resident.py` (the window header speaks first: name the window, list what is here, name no other, never open with a denial) · `docs/rearch/0005` (P5 CLOSED WHOLE; sp1 walked) · `docs/rearch/0007` (walk #6; the honest limit) | Fable 5.1 (claude-fable-5-1); a fresh Fable 5.1 Playwright subagent walked | REARCH — WALK #6 (lean): INTENT-01 PASS ×4 · ANALYZE-01 PASS ×2 · SCOPE-01 PASS (the mind names the window's seven asks — walk #5's wound CLOSED) · SESSION-01 short version PASS · MARK-01's 4th bar PASS (the critique in the chat in 16 s — CLOSED) → **PHASE 5 CLOSES WHOLE**. Four wound calls ruled: the crew's reply reaching the chat (cured) · whose word (cured) · SCOPE-01's opening denial + an invented ask (the header now speaks first; verify at the next walk) · the librarian has no tools for Resiliency's objectives (sp1's honest limit, recorded in 0007). |
| `docs/rearch/0008-the-rust-plane-and-the-port.md` (new, LOCKED — kernel Rust by P7, bodies LangGraph, glass one page; the conformance-suite law; the sequence) · `spine/conformance/README.md` + `spine/conformance/envelope-v0.json` (new — the first fixture, 9 cases, generated FROM the Python reference) · `spine/tests/test_conformance.py` (new — every fixture runs on every CI pass) · `docs/rearch/0005` (0008 in the checklist · P7 = the port · JB walks the glass · fixtures in the close conditions) · `docs/rearch/0007` + `experience-capture.md` ("not yet", by sequence; JB's correction recorded) · `docs/rearch/playwright-agent-charter.md` (RETIRED, with the numbers) | Fable 5.1 (claude-fable-5-1) | REARCH CANON 0008 + the conformance suite's first fixture — JB's three locks 2026-09-21: P6 in Python, P7 = the port · JB tests in the glass, Fable keeps the register · walk #6 committed (2df948b). The Saturday token bleed diagnosed from the transcripts (390k main ctx × 53 turns + 196k walker × 127 turns + the memory observer ≈ 36M tokens in 30 min; the walker ran on after "done"). |
| `orreth_spine/outbox.py` (the ground memo: `once` runs DDL once per GROUND per process — a door's fresh connection is born flagged; `ground_key` = DSN + search_path, asked safely in every transaction state) · `orreth_spine/ground.py` (the law sharpened) · `tests/test_intent.py` (born flagged; two grounds stay two) | Fable 5.1 (claude-fable-5-1) | REARCH — CI wound cured: three red runs since 135d975 (local green): `test_band_fanout` deadlocked at `pg_advisory_xact_lock` because every door request re-ran the schema DDL on a fresh connection (ALTER TABLE … IF NOT EXISTS takes AccessExclusiveLock even when the column exists) while a resident served. Suite 103. |
| `docs/rearch/0005` (Phase 6 GOVERNANCE FELT opened: sp1 L3 · sp2 compliance export w/ AG-7 · sp3 MITL v0 + the impact door · sp4 placement v0; close conditions; what P6 is not) · `docs/rearch/experience-specs.md` (SPEC-L3-01 · EXPORT-01 · MITL-01 · PLACE-01, planned) | Fable 5.1 (claude-fable-5-1) | REARCH — P6 OPENED 2026-09-21 on JB's "P6 my friend"; slicing is Fable's proposal, JB may veto at sight; sp1 builds in a fresh session (the token law). |
| `orreth_spine/proof.py` (new — TOTP RFC 6238 pure Python · the ladder · the authenticator on the ground, enrolled through the chat, re-enrolling grave · masters seeded from `SPINE_MASTERS` · every proof a recorded attempt · the kernel's own held acts) · `orreth_spine/tools.py` (consequence classes; `ConsequentialHold` wears class + level; test-only grave `erase-record`) · `orreth_spine/resident.py` (the hold wears its level; `spine_asks.proof`; the reply's envelope carries `proof`; the kernel's rest reason) · `orreth_spine/dispatch.py` (`confirm_ask` judges the proof — ONE face, three wrongs rest the act, the kernel's acts settled on the ground) · `orreth_spine/intent.py` (a bare stop on a kernel intention raises `ProofRequired`; the proven stop carries its level) · `orreth_spine/glass.py` (`/enroll` · `/enroll/confirm` · `GET /proof` · `/confirm` with `by`+`code` → 403 one face · `/intentions/stop` holds the kernel's · `ask_view` shows `proof` + `hold`) · `orreth_spine/ground.py` (the `proof` ground) · `glass/index.html` (L3-code and L3-master rows, Enter never submits, cancel default; "enroll my authenticator" draws the QR; "stop the kernel's … intention"; proof on the journey line) · `conformance/proof-v0.json` (new, 14 cases) + `tests/test_conformance.py` + `conformance/README.md` (new case kinds) · `tests/test_proof.py` (new, 9 laws) · `tests/test_intent.py` (the bare kernel stop replaced by the proven one) · `pyproject.toml` (+ `qrcode`, pure Python; the PNG written by hand) · `docs/rearch/0005` (P6 sp1 BUILT, walk owed) | Fable 5.1 (claude-fable-5-1) | REARCH — P6 sp1 L3: THE PROOF DEMAND RISES, built 2026-09-21. Suite 126 green (9 sp1 laws). Walk OWED: SPEC-L3-01 in the glass. |
| `orreth_spine/export.py` (new — the compliance bundle `orreth.compliance/1`: one scope (session · window · marker root · the current session by default), every ask/hold/proof/reply/include/intention-stop/marker-set row wearing the chain its envelope carried, `chain_status` judged from the row alone and counted, proof level, marker lineage, words (none for opt-out), the sha256 hash chain + `verify`, Ed25519 signing when a signer is given, the CSV with the 500-char cut marked) · `orreth_spine/identity.py` (`verify_key_hex`) · `orreth_spine/glass.py` (`GET /export` json|csv — a READ) · `glass/index.html` ("export compliance …" words: this session · the recall's window words · an origin by its words; the table in place; download JSON/CSV as Blobs; plain words — "the chain of who acted for whom") · `conformance/export-v0.json` (new, 6 cases) + `tests/test_conformance.py` (`hash_chain` · `chain_status` · `verify` kinds) + `conformance/README.md` · `tests/test_export.py` (new, 10 laws) · `docs/rearch/0005` (P6 sp2 BUILT, walk owed) | Fable 5.1 (claude-fable-5-1) | REARCH — P6 sp2 THE COMPLIANCE EXPORT WITH AG-7 CHAINS, built 2026-09-21. AG-7 PROVEN in the suite: the planned request's chain H → residents read → firmware unbroken on every include row; a cut hop fails `verify`. Suite 141 green (10 sp2 laws). Honest: unsigned at the door (no kernel self yet — seam), another person's words = a named seam (not built), the tool hop is NOT on the wire today (`spine_tool_calls` carries no chain/event — a real AG-7 finding). Walk OWED: SPEC-EXPORT-01. |
| `orreth_spine/tools.py` (the door wears the chain: `ask` + `chain` on `ToolDoor`, `authority_chain` + `ask` columns on `spine_tool_calls` via its once-guarded `ensure_schema`, the `orreth.tool.called.v1` fact through the outbox with the chain end to end (`… → tool:<name>`) and the action marker; the mark tool passes the chain on) · `orreth_spine/markers.py` (`set_marker(chain=…)`: the fact wears the setter's full chain, the origin human first) · `orreth_spine/resident.py` (`_current_chain` + `_full_chain`: ONE chain for journey · reply · hold · every tool call; the door gets ask + chain at think and at the released hold, the released act under its ask's marker) · `orreth_spine/export.py` (`tool` rows from the fact; a body's mark row: person = the ask's human, chain ends with the setter) · `conformance/export-v0.json` (+3 cases: the four-row chain with the tool hop, the tool row's status, the tool-called envelope's bytes) · `tests/test_export.py` (+1 law, 11) · `docs/rearch/0005` (the wound cured) | Fable 5.1 (claude-fable-5-1) | REARCH — P6 sp2b THE TOOL HOP WEARS THE CHAIN (the AG-7 wound sp2 found, cured the same day): a tool row of the export reads H → resident → tool:<name> from the record, never rebuilt; a cut tool hop fails `verify`. Suite 145 green. Seams: a tool has no self — `tool:<name>` names the hop; a released hold wears the confirm command's chain (H → body → tool), the read hops stay on the hold row. |
| `orreth_spine/outbox.py` (`_GROUNDS_DONE` + `mark_ground_done`: a ground counts as ensured only when a BIRTH has FINISHED) · `orreth_spine/ground.py` (ensure_all marks done) | Fable 5.1 (claude-fable-5-1) | REARCH — CI race cured (57cf0b1 attempt 1: a rig thread born mid-birth skipped the markers DDL, its seed hit an undefined table, the relay thread died, no resident replied). Suite 145. |
| `orreth_spine/mitl.py` (new — MITL v0: the Orreth ontology acquired through the librarian's `acquire` door in provenance-bearing passages; the soft toggle as recorded facts `orreth.mitl.summoned.v1` / `.dismissed.v1` with the chain and a marker; the impact door — the ground read by rule, the verdict by sp1's ladder, the words asked of MITL as a thought filed under the change's marker; `orreth.impact/1`) · `templates/firmware-mitl.v0.json` (new) · `orreth_spine/resident.py` (MITL's own system words; its recall packs the canon passages that match the ask) · `orreth_spine/dispatch.py` (a BODY never confirms — one face) · `orreth_spine/glass.py` (mitl born with the rig, the ontology at birth; `GET/POST /mitl` · `POST /impact`) · `orreth_spine/ground.py` + `markers.py` (`mitl` ensured; in INCLUDES) · `glass/index.html` (the MITL chip beside KIND, "summon/dismiss MITL", "what does the canon say …", "expected impact?" on every hold and on an intention about to be declared, the answer beneath the confirm) · `conformance/mitl-v0.json` (new, 9 cases) + `README.md` · `tests/test_mitl.py` (new, 6 laws) · `tests/test_crew.py` (the roster names mitl) · `docs/rearch/0005` (sp3 BUILT) | Fable 5.1 (claude-fable-5-1) | REARCH — P6 sp3 BUILT 2026-09-21; suite 160 green; relit once (139 passages at birth, `/impact` answered through the meter); walk OWED (SPEC-MITL-01). Factories out; placement (sp4) not built. |
| `orreth_spine/placement.py` (new — placement policy v0: the profile with defaults, the ground declaring itself (`SPINE_CELL` · `SPINE_METAL` · secret NAMES), the honor rule with every unmet clause in words, the refusal as a fact `orreth.body.refused.v1` + `spine_refusals`) · `orreth_spine/resident.py` (enforced at the join — refused, never started; `spine_joins.placement`; the echo fallback's persona default) · `orreth_spine/glass.py` (crew cards wear where-and-why; refused cards greyed with the reason; the serve loop ends a refused seat and the rig runs on) · `orreth_spine/export.py` (`placement` on every row + CSV column) · `orreth_spine/mitl.py` (kind `placement` reads the profile, judges it against this ground, consequential → L2) · `orreth_spine/ground.py` (`placement` ensured) · `glass/index.html` (the stands line, secrets by name with a reached mark, the refused card, "where does the librarian stand?") · `templates/firmware-{planner,critic,grader}.v0.json` (a one-line persona each — sp3's wound) · `conformance/placement-v0.json` (new, 11 cases) + `README.md` + `tests/test_conformance.py` (`profile` · `honor`) · `tests/test_placement.py` (new, 7 laws) · `tests/test_mitl.py` (the "not built yet" note replaced by name) · `docs/rearch/0005` (sp4 BUILT) | Fable 5.1 (claude-fable-5-1) | REARCH — P6 sp4 BUILT 2026-09-21; suite 178 green; relit once (eight bodies seated, every card "stands on local · cpu"); walk OWED (SPEC-PLACE-01). One cell until P7; affinity advisory; secrets by name; allen/security/warden not bodies yet. |
| `orreth_spine/monitor.py` (W14 — the sense of a watch: RED when `metric op threshold` holds; `judge()` records red ↔ green with `since` and the fact `orreth.watch.turned.v1`; `judge_one` · `reads`; snapshot watches carry `red` · `state` · `since` · `reads`) · `orreth_spine/intent.py` (the loop wakes on `monitor.judge`'s red transitions; W8 `_hear_runners` — a runner's `CANNOT ACT:` marks ONE improvement and blocks the intention until `crew_hash` changes; W5 `stop_demand` — ANY stop is grave: L3-code for a human's/role's, L3-master + `needs_code` for the kernel's; `blocked` · `blocked_note` on rows) · `orreth_spine/proof.py` (`ProofRequired.needs_code`; `hold_kernel_act(needs_code=)` → the hold's `needs_code` · `code_ok`; `question_for` says code-then-master; settle words per level; `replied_at = clock_timestamp()`) · `orreth_spine/dispatch.py` (W19 `absent` · `refusal_words` · `_refuse_at_door` — status `refused`, the fact `orreth.ask.refused.v1`, the marker kept; W5 `confirm_ask` — the asker's code step `{step: code, next: master}` before the master's click; `zone` on the ask) · `orreth_spine/resident.py` (W12 `seat_words` — UTC now, the human's zone (`SPINE_HUMAN_ZONE`, default America/Denver), the world, the covenant slice, the template's `ontology` slice from MITL's corpus, the common laws; W8 the pack labels ANOTHER BODY'S WORDS; W17 `replied_at = clock_timestamp()`; `zone` columns) · `orreth_spine/gateway.py` (the meter's `at` = clock_timestamp()) · `orreth_spine/glass.py` (the hold view wears `needs_code` · `code_ok`; `/intentions/stop` returns `needs_code`; `/ask` takes `zone`; `ASK_REFUSED` + `WATCH_TURNED` on the feed) · `orreth_spine/export.py` (kind `refused`) · `orreth_spine/tools.py` + `bindings/monitor.v0.json` (the watch's sense and CALL-the-tool in the seat's words) · `conformance/watch-v0.json` + `ask-v0.json` (new) + `proof-v0.json` (`stop_demand`) + `README.md` + `tests/test_conformance.py` · `tests/test_cure_walk7.py` (new, 8 laws) · `tests/test_intent.py` · `test_monitor.py` · `test_proof.py` · `test_mitl.py` · `test_markers.py` (replaced by name: the watch's sense, the bare human stop, the master before the code, MITL's exact words, bodies joined before the door asks them) · `docs/rearch/baselines/after-walk-2026-09-17.md` (cures landed) · `docs/rearch/0005` (P6 cure sp1) · `docs/rearch/0007` (IH-1, the honest limit) | Fable 5.1 (claude-fable-5-1) | REARCH — P6 cure sp1 (kernel) BUILT 2026-09-21; suite 202 green (178 → 202); relit once: the three live watches read green · red · green as their metrics say (the board had stood RED at 0 and 5). W16's premise corrected on the ground (the meter row exists); W14 was a sense inversion, not a stuck state. Glass half owed to the next builder. |
| `spine/glass/index.html` (walk #7's glass cures: `codeRow` · `holdRow` · `stepDone` · `stepRefused` · `drawEnroll` · `enrollWords` re-enroll through the code row, the QR alone with the secret behind a click; `stopIntention` without a second click, `decide` reading `{step: code, next: master}`; `foldTree` and the OBJECTIVES/OBSERVATIONS bands, `blocked` on intention rows; `addressed` + `zone` on every ask; `copyBtn` · `stripMarks` · `cite`; `loadRoster` on `/crew` with `nature` and refused chips; the export table's why in words, target, "refused at the door"; `layoutChat` + `.raised`; `onNotice` for `orreth.watch.turned.v1` and `orreth.ask.refused.v1`; the monitor's "propose it") · `orreth_spine/resident.py` (`Resident.nature` from the template or the binding; `spine_joins.nature`) · `orreth_spine/glass.py` (`crew_view` · `_refused_card` · `residents_view` carry `nature`; `ask_view` carries `offer`; `address_to` at the `/ask` door; `/mitl` carries `citations`) · `orreth_spine/dispatch.py` (`address`) · `orreth_spine/monitor.py` (`offer_in`) · `orreth_spine/mitl.py` (`TITLES` · `citation_name` · `citations`; the impact ask starts with the change) · `orreth_spine/markers.py` (`with_words` carries `blocked` · `blocked_note`) · `orreth_spine/export.py` (rows carry `marker_words` — never from an opt-out session — and `target`; `_plain`; CSV +`target` · `marker_words`) · `spine/templates/*.json` + `spine/bindings/*.json` (`nature`) · `spine/conformance/{ask,watch,mitl}-v0.json` (+`address` · `offer` · `citation_name`) · `spine/conformance/README.md` · `spine/tests/test_cure_walk7_glass.py` (new, 8 laws) · `spine/tests/test_conformance.py` · `spine/tests/test_placement.py` (the CSV's last-column assertion loosened to presence — W11 grew two columns) · `docs/rearch/0005-v1-scope-and-build-plan.md` · `docs/rearch/baselines/after-walk-2026-09-17.md` | Fable 5.1 (claude-fable-5-1) | REARCH — P6 cure sp2 (glass) BUILT 2026-09-21; suite 220 green (202 → 220); relit once: `/crew` carries every body's nature, `/proof` answers, `/mitl` carries the citation names, the watches read state · since · reads; the glass behaviour is JB's to walk (L3-01 · MITL 3.3 re-walk owed). Verified by JB against the transcript's model IDs. |
| `orreth_spine/intent.py` (W20 — `restart` · `restart_demand` · `INTENTION_RESTARTED`, the history whole) · `orreth_spine/proof.py` (the kernel settles `intent.restart`; W23 the L3 question's words) · `orreth_spine/scheduler.py` (W21 — `duty_text` · `cadence_words` · `notes_for`; the occurrence framed as a duty) · `orreth_spine/resident.py` (W21 `DUTY_LAW` + the pack's OWN NOTES · W23 `interlock_words` · W24 `echo_reply`, the assertion in the journey) · `orreth_spine/monitor.py` (W22 `offer_in` without backticks, `PROPOSE_BARE`) · `orreth_spine/harness.py` (the world checks: `refused_words` · `duty_answered` · `offers_are_holds` · `checks`) · `orreth_spine/markers.py` (the schedule row's cadence · runs · last) · `orreth_spine/glass.py` (`POST /intentions/restart` · `GET /harness`) · `spine/bindings/monitor.v0.json` (the proposal law) · `spine/glass/index.html` (restart on the rested row and by words · `actOnIntention` · the duty fold `dutyBubble` · the schedule row's cadence · the hold's words STANDS/stays AT REST · "propose it" without words · W24 `readWindow`) · `spine/conformance/intent-v0.json` (new) + `watch-v0.json` (+2 offer, 1 replaced by name) + `README.md` · `spine/tests/test_cure_walk8.py` (new, 7 laws) · `test_conformance.py` (4 kinds) · `test_scheduler.py` + `test_cure_walk7_glass.py` (one line each replaced by name) · `docs/rearch/0005` (P6 CLOSED WHOLE · cure sp3) · `docs/rearch/experience-specs.md` (L3-01 · EXPORT-01 · MITL-01 · PLACE-01 → PASS) · `docs/rearch/baselines/after-walk-2026-09-17.md` (walk #8 cures landed) | Fable 5.1 (claude-fable-5-1) | REARCH — P6 cure sp3 (the re-walk's wounds) + close staging, 2026-09-21: W20 the restart (a new fact through the stop's ladder) · W21 a duty framed as a duty, never refused · W22 an offer is a proposal · W23 the interlock's words (rule 11) · W24 a body's nature on its chip · the hold's words · two harness checks on `GET /harness`. Suite 254 (220 → 254). Relit once: the restart door held JB's rested intention at L3-code; both checks ok. Verified by JB against the transcript's model IDs. |
| `backend/plane/crates/orreth-spine/` (new — the Rust kernel's pure laws, P7 sp1: `Cargo.toml` · `src/lib.rs` · `canonical.rs` (Python's `ensure_ascii` escaper + float `repr`, by hand) · `hash.rs` · `envelope.rs` (encode · decode · refusals by name) · `proof.rs` (TOTP RFC 6238 · the ladder · stop/restart demand) · `export.rs` (hash chain · chain_status · verify incl. Ed25519) · `watch.rs` (judge · reads · offer_in) · `placement.rs` (profile · honor · why_here) · `ask.rs` (address · refusal_words · refused_words · echo_reply · cadence_words · duty_text) · `mitl.rs` (citation_name · verdict) · `py.rs` (Python truth/str) · `tests/conformance.rs` (the Rust fixture runner: every `spine/conformance/*-v*.json`, dispatched by kind, coverage reported, unported kinds named)) · `backend/plane/Cargo.toml` (workspace member + `hmac` · `sha1` · `regex`) · `Cargo.lock` · `docs/rearch/0005` (P7 sp1 BUILT) · `docs/rearch/0008` (the runner named) | Fable 5.1 (claude-fable-5-1) | REARCH — P7 sp1 THE BYTES, built 2026-09-22: ported 105/105 cases, all 25 kinds, not yet ported none; the canonical crux byte-identical to Python (surrogate pairs, DEL, float repr — 16 forms proven against CPython); `cargo test` green across the workspace, the five sacred crates untouched; clippy clean. JB's review of the plane diff OWED before the push (0008 rule 5). Honest: the Ed25519 verify branch proven by a Python-signed bundle in a unit test (fixture unsigned); floats beyond the proven forms rest on shortest-digits agreement; no ground/broker/clock/door — sp2's. |
| `backend/plane/crates/orreth-spine/` (P7 sp2 — the ground and the rails behind the `rails` feature: `src/rails.rs` (PURE names + shapes: queues · keys · topics · the outbox row · the inbox road) · `ground.rs` (connect; the once-guard, the process memo, born-flagged after a finished birth, advisory lock 742199) · `outbox.rs` (`commit_with_outbox` · `relay_once` · `drain` · the budget by name · `MemorySink`) · `inbox.rs` (`apply_once` · `apply_event` · the gap by name) · `sinks.rs` (`KafkaSink`) · `invoke.rs` (RabbitMQ: declare · publish with confirms · receive + ack) · `events.rs` (declare topics · `Reader`) · `heartbeat.rs` (three breaths + all three composed) · `rail_error.rs` · `envelope.rs` (`iso_millis` · `now_iso` · `Mint`) · `lib.rs` · `Cargo.toml` (feature + optional deps + `[[test]] rails` required-features) · `tests/rails.rs` (new, 5 rig tests incl. the SHADOW) · `tests/conformance.rs` (+ `rail_names` · `outbox_row` · `inbox_key`)) · `backend/plane/Cargo.toml` (workspace deps: tokio · tokio-postgres · lapin · rdkafka `cmake-build` · rand) · `Cargo.lock` · `scripts/dev.sh` (rewritten Docker-first for the NEW rig: up · down · status · logs · bridge [stop] · suite · rust [rails] · walk; the OLD rig verbatim under `old <verb>`; `replant` kept top-level for the keeper) · `spine/conformance/rails-v0.json` (new, 12 cases, generated by the Python reference) + `README.md` + `spine/tests/test_conformance.py` (three kinds; `_dials`) · `docs/rearch/0005` (P7 sp2 BUILT) · `docs/rearch/0008` (the first shadow) | Fable 5.1 (claude-fable-5-1) | REARCH — P7 sp2 THE GROUND AND THE RAILS, built 2026-09-22: 5/5 rail tests green on the rig (ground 2.9 ms · invoke 69 ms · events 300 ms; the SHADOW proven both ways on the public ground); conformance Python 118 passed · Rust 117/117, 28 kinds, none unported; `cargo test` green across the workspace (hermetic; the feature off), clippy clean, fmt'd; the five sacred crates and `orrethd` untouched. dev.sh verbs each run once (down not exercised — the rig was shared). JB's review of the plane diff OWED before the push (0008 rule 5). Honest: CI has no rig — rail tests skip BY NAME there; the dispatcher loop and the parking projector are sp3's. |
| `backend/plane/crates/orreth-spine/` (P7 sp3 — THE ASK ROAD in Rust, in SHADOW: pure `services.rs` (ladder_step · pin) · `intent.rs` (P23 read_words) · `ask.rs` (the ask's fact + the refusal as bytes) · `proof.rs` (otpauth_uri · question_for · KERNEL · one_face); behind `rails`: `schema.rs` (the road's six tables, Python's DDL word for word) · `world.rs` (dials · RoadError · isoformat) · `markers_live.rs` · `proof_live.rs` (enroll/confirm/masters/judge; the QR by `qrcode` + a hand-written PNG) · `asks.rs` (submit_ask · absent · address_to · confirm_ask · ask_view · asks_view) · `sessions.rs` (sessions · residents · crew · the shelf's read) · `intent_live.rs` (declare · listing) · `dispatcher.rs` (the standing dispatcher + relay; THE SHADOW LAW: one inbox consumer name hands each fact to exactly one) · `feed.rs`; behind `bridge`: `bridge.rs` (axum 0.7 doors + SSE feed) · `src/bin/spine-bridge.rs` (:4601); `events.rs` grew `assigned` · `next` / `commit`; `ground.rs` ensures the road's tags at birth; `tests/askroad.rs` (the rig-gated shadow proof); `tests/conformance.rs` + `spine/tests/test_conformance.py` + `spine/conformance/askroad-v0.json` (18 cases) · `Cargo.toml` (workspace: axum · tokio-stream · qrcode · flate2) · `scripts/dev.sh` (`shadow` / `shadow stop`; status line) · `docs/rearch/0005` · `0008`) | Fable 5.1 (claude-fable-5-1) | REARCH — P7 sp3 THE ASK ROAD, built 2026-09-22: the shadow proof green on the rig (an ask through the Rust door → the RUST dispatcher (meter 1, one inbox footprint) → the PYTHON librarian → the reply equal at :4601 and :4600; W19 · fan-out · one face · sessions · the feed); relit by hand beside the lit Python Bridge (views · analyzer · crew EQUAL; both dispatchers alive, the inbox let Rust absorb — never a double dispatch); conformance Python 158 passed, Rust 157/157 cases · 35 kinds · not yet ported none; `cargo test` green across the workspace; clippy clean with and without the features; the five sacred crates and `orrethd` untouched. Honest limits: the loops are sp4's, the digest sp5's, a kernel-held APPROVE answers 501 by name, unported doors 404 — all named in 0005. JB's review of the plane diff OWED before the push. |
| `orreth_spine/services.py` (new — the services registry v0: ONE registry for tool · mcp · store · source · mind, each a `did:orreth:service:…` self with a persisted seed (rule 1), the manifest pin, placement honored at register, secrets by NAME (0059: a key in ZERO records; an unreachable secret refuses by name and records nothing), ONE ladder registered → versioned(n) → healthy \| unhealthy → retired with `ladder_step` the pure law, retired = dormancy never deletion, restore a new fact, every step a fact through the outbox with chain + marker under ONE shelf origin per world; retire held at L2 as the kernel's own act; health by kind (tool: describe + pin · mind: a one-token ping through the meter under the service's DID · store/source: the locator by NAME · mcp: "not yet probed"); the built-ins seeded at boot) · `orreth_spine/identity.py` (`kind` — a service is a self too) · `orreth_spine/tools.py` (`tool_manifest`; `spine_tool_calls.service`; the hop wears the service DID, `tool:<name>` the fallback; a retired tool refuses with a teaching) · `orreth_spine/gateway.py` (one `meter()` for every lane; `spine_meter.service` = the mind service's DID; the fake lanes name their model) · `orreth_spine/export.py` (the tool row's `served_by` reads the service DID) · `orreth_spine/proof.py` (`question_for` L2; the kernel settles `service.retire`) · `orreth_spine/harness.py` (the check "every service healthy or retired") · `orreth_spine/mitl.py` (change kind `service`; `touches.services`; the declaring bodies named) · `orreth_spine/ground.py` (`services` ensured) · `orreth_spine/glass.py` (`GET /services[?kind]` · `POST /services` · `/version` · `/check` · `/retire` (202 held) · `/restore`; the rig seeds and probes the shelf at boot; the services' seeds beside the agents') · `spine/glass/index.html` (THE SHELF in the CREW pull; "what services / tools / minds are here?" · "retire the weather tool" → the interlock · "restore …" · "check the services"; `impactKind` service) · `spine/compose.yaml` (the ground on the NAMED volume `orreth-spine-ground` — JB's lock; migrated once by hand, counts verified) · `spine/conformance/services-v0.json` (new, 22 cases: `ladder_step` · `manifest_pin` · 2 `encode`) + `README.md` + `tests/test_conformance.py` · `spine/tests/test_services.py` (new, 7 laws) · `spine/tests/test_cure_walk8.py` (the harness list names the third check — replaced by name) · `docs/rearch/0005` (P6.5 sp1 BUILT, walk owed) · `docs/rearch/0009` (0018 services-as-identities → CARRIED partial; 0059 env-secrets → CARRIED; the roster law for services; tally re-counted) | Fable 5.1 (claude-fable-5-1) | REARCH — P6.5 sp1 THE SERVICES REGISTRY, ONE LADDER, built 2026-09-22 beside the port (JB's lock: one ladder, two keepers). Suite 295 green (140 conformance cases within); the Rust runner reports `ladder_step (16) · manifest_pin (4)` as not yet ported, honestly, and stays green. One wound found in the suite and cured: nine health probes per rig boot were each a root observation and pushed Resiliency off the Analyzer's newest-60 — now ONE shelf origin per world, every step beneath its service. The ground's data migrated to the named volume (row counts identical before/after; the anonymous volume `4ac4d0fb…` left for JB to remove). Walk OWED: SPEC-SERVICES-01 in the glass. Honest limits: mcp health sp2's; minds beyond the lit route sp3's; a service signs nothing yet; the harness reads the last health, it runs no probe. |
| `orreth_spine/mcp.py` (new — MCP through ONE door: the kernel's Model Context Protocol client by hand, JSON-RPC 2.0 over stdio (a spawned command) and streamable HTTP (a URL; SSE unwrapped) — `initialize` · `tools/list` · `tools/call`, a session per act, canonical request bytes with fixed ids; a server as a service of kind `mcp` with its locator BY NAME, its listed tools as services of kind `tool` placed under it, the same DIDs every register; `spawn_env` — the child's environment from NAMES only (the env-secrets law); a changed list versions the server, a new tool registers, a vanished tool goes unhealthy "gone from the server's list" (never retired by the machine); `probe` · `probe_tool` · `call` · `changes` · the strikes rule (`strikes` · `propose_retire` · `proposals` · `keeper_beat`) · the reference clock by the dial `SPINE_MCP_REF`) · `spine/mcp_ref/clock_server.py` (new — the reference MCP server, stdlib only: `now` · `echo`, its listing shaped by the env NAME `SPINE_MCP_REF_TOOLS`) · `spine/templates/firmware-toolkeeper.v0.json` (new — the Tools keeper, third kind, function `tools`, one tool `services`) · `orreth_spine/tools.py` (the `services` tool — register · check · version · retire · restore · changes · list; `consequence_of(tool, args)` reads a class BY THE ARGUMENTS (`consequence_by`); the ToolDoor lists and dispatches MCP-born tools through `tools/call`; `_ask` rides a ground tool's args) · `orreth_spine/services.py` (the mcp probe is real; `record_health` — a verdict without a probe; `HOME` — the rig's seed home for the keeper's door; the confirmer rides the retire fact's chain) · `orreth_spine/proof.py` (the kernel settles `service.retire` with the confirmer on the chain) · `orreth_spine/glass.py` (the toolkeeper born with the rig; the keeper's beat on the scheduler's clock, `SPINE_TOOL_CHECK_S`; the reference clock seeded by the dial; `POST /services/mcp`) · `orreth_spine/harness.py` (two checks: every MCP server answers initialize · the keeper proposes after strikes, never retires alone) · `orreth_spine/mitl.py` (an MCP server's change names the tools under it; a tool names its server) · `spine/templates/librarian-resident.v0.json` (`tools:now`) · `spine/glass/index.html` (THE SHELF: MCP servers with their tools nested beneath, the locator by name, the tool count, the last list time; a vanished tool's reason; the keeper's proposals arrive in the chat as holds) · `spine/conformance/mcp-v0.json` (new, 15 cases: `mcp_request` · `mcp_tool_manifest` · `mcp_server_manifest` · `mcp_transport` · `mcp_words`) + `README.md` + `tests/test_conformance.py` · `spine/tests/test_toolkeeper.py` (new, 8 laws) · `spine/tests/test_services.py` · `test_cure_walk8.py` · `test_crew.py` (three assertions replaced by name: sp1's "not yet probed", the built-ins set, the harness list and the crew roster) · `docs/rearch/0005` (P6.5 sp2 BUILT, walk owed) · `docs/rearch/0009` (the MCP row 0059 → CARRIED; 0018 and the env-secrets rows re-read; tally re-counted) | Fable 5.1 (claude-fable-5-1) | REARCH — P6.5 sp2 THE TOOLS KEEPER: MCP THROUGH ONE DOOR, built 2026-09-23 beside the port (JB's lock: one ladder, two keepers — the first keeper stands). Suite 336 green (173 conformance within, 295 → 336); the Rust runner reports the five `mcp_*` kinds (15 cases) as not yet ported, honestly, and stays green (`ported 157/172`). Relit once with `SPINE_MCP_REF=1`: the toolkeeper born beside mitl, the clock server registered with `now` and `echo` healthy under it, all five harness checks true. Honest limits: stdio + streamable HTTP, tools only, no OAuth, no standing session, the strikes rule v0. Awaits JB's walk (SPEC-TOOLS-01). |
