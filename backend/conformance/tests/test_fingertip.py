...
the human at the top. Templates refused-at-save; slivers carry nothing extra;
...
def test_workflow_template_gates_refuse_at_save():
    ...
    spec = fingertip.workflow_template(
        ...
        slivers=[{"id": "design", "intent": "draft the approach"}])
    ...
    headless = fingertip.workflow_template(
        "u:demo", name="x", slivers=[{"id": "a", "intent": "b"}])
    ...
    standing = fingertip.workflow_template(
        ...
        slivers=[{"id": "watch", "intent": "review the floors"}])
...
# ---------------------------------------------------------------- the sliver contract
def test_sliver_carries_the_sliver_and_nothing_else():
    """0027 §3: one intent, a budget slice, citation refs — never the plan,
    never the siblings, never the why."""
    s = fingertip.make_sliver("sha256:goal", "test the widget", 300, ["sha256:ref1"])
...
def test_objective_dispatches_reviews_and_assembles(world):
    """The universe plans high, works low, reviews on the way back up — slivers
    ...
    # the fingertips were the sliver's life — retired after it
...
def test_hitl_question_parks_the_branch_until_answered(world):
    ...
    spec = fingertip.workflow_template(
        ...
        slivers=[{"id": "build", "intent": "compile the artifact"},
    ...
    first = orch.run(seats, beckys, _tidy_think)
    assert first["verification"] == "partial"
    assert first["waiting_on_human"] == ["iac"]
    assert orch.questions() == [{"sliver": "iac",
    ...
    second = orch.run(seats, beckys, _tidy_think)
    assert second["verification"] == "complete" and not orch.questions()
    iac = next(b for b in second["branches"] if b["sliver"] == "iac")
...
def test_cross_ecosystem_dispatch_refuses_and_asks(world):
    ...
    spec = fingertip.workflow_template(
        ...
        slivers=[{"id": "local", "intent": "do the local part"},
    ...
    dark = next(b for b in out["branches"] if b["sliver"] == "foreign")
    ...
    assert not any("sliver-outcome" in r.get("tags", [])
