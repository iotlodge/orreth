...
- **sp4 — THE LOOPS** ✅ **BUILT 2026-09-23 — JB's review of the plane
  ...
  **THE BEAT LOCK — the loops' shadow law (new, both spines):** a beat reads
  what is due and acts on it, and two kernels beating one world at once
  would file the same occurrence twice, observe the same red twice, plan the
  same cause twice. So a beat is CLAIMED on the ground before it runs — a
  session advisory lock keyed by the beat's class and the world's scope
  (`pg_try_advisory_lock(742200 + class, hashtext(scope))`; scheduler 1 ·
  intent 2 · keeper 3), held for the beat, released after; the kernel that
  finds it held steps back and says so (`"the beat is held by another kernel
  on this ground"`); a kernel that dies mid-beat drops it with its
  connection. Python: `ground.beat` / `try_beat` / `end_beat`, and
  `scheduler.tick` + `intent.turn` run under it (a test: one connection
  holds, the other's tick returns nothing, its turn says so; per class, per
  world; a closed connection frees it). Rust: `beat.rs`.
  **What stands in Rust** (`orreth-spine` +~2,900 lines): pure — `mcp.rs`
  (the five mcp kinds: the requests' bytes with fixed ids, the transport,
  `shelf_name`, the tool and server manifests, the words, the strikes dial) ·
  `beat.rs` (the classes, the keys, the words) · the loop's words in
  `intent.rs` (`plan_words` · `observed_words` · `watch_note` with Python's
  `repr` quoting · `cannot_act` · `improvement_note` · `crew_shape_hash` ·
  `resiliency()` · `CADENCE_DUE`) · the turned fact in `watch.rs` · W26's
  words and `LEASE_TTL_S` in `ask.rs` · `py::repr_str`; behind `rails` —
  `schema.rs` grows the loops' three tables (Python's DDL word for word:
  `spine_watches` · `spine_schedules` + `spine_occurrences` ·
  `spine_harness_runs`; ground tags 12) · `presence.rs` (`renew` · `roster`)
  · `monitor.rs` (`add_watch` · `snapshot` with the benches by passive
  declare and the topic's watermarks · `judge` — the transition recorded with
  `since` and the `watch.turned` fact in one transaction) · `scheduler.rs`
  (`add` · `declared` · `rest` with the kernel-required face · `for_runner` ·
  `notes_for` · `clock` — the last run's time in the human's zone read off
  the ground's own tzdata, `to_char(… AT TIME ZONE …, 'FMHH12:MI AM')` · the
  `tick` under the beat: every due human/role schedule occurs as a duty-framed
  ask; the kernel's "run the harness" duty is LEFT DUE for the kernel that
  holds the bodies) · `harness.rs` (the five world checks, the strikes read)
  · `intent_live.rs` (`stop` · `restart` — one function `stop_or_restart_in`
  inside a caller's transaction, the proof demanded as `RoadError::
  ProofRequired` · `declared` · `interested` · `plan` · `on_marker` ·
  `crew_hash` · `hear_runners` (W8) · `due` · `file_objectives` · the `turn`
  under the beat) · `markers_live.rs` (`set_marker` with its fact ·
  `interested_bodies` · `dispatch_interests` — the interest law at body and
  intention level) · `proof_live.rs` (`hold_kernel_act`) · `asks.rs` (the
  APPROVE of a kernel-held act now RUNS `intent.stop` / `intent.restart` in
  the record's own transaction; `service.retire` stays 501 by name — the
  organ's seam). **The bridge:** two standing tasks — the scheduler's beat
  every 5 s, the intent rail's beat every 3 s with Resiliency declared at boot
  (once per world; one at rest stays at rest) — and the doors `GET /monitor`
  · `/schedules/<runner>` · `/harness`; `POST /schedules` · `/schedules/rest`
  · `/intentions/stop` · `/intentions/restart` (a bare stop HELD for the
  code; the kernel's for code-then-master); `/harness/run` 501 by name.
  **W26 CURED (Python serve path, `resident.py`):** a mind that returns no
  words is asked again ONCE (a journey step names it); a second silence lands
  as `"the mind returned nothing; asked again, nothing"` with status
  `replied` — never an empty bubble; both thoughts metered (a test with a
  scripted silent mind).
  **Conformance:** `loops-v0.json` (new, 23 cases, generated by the Python
  reference): `beat_lock` · `loop_words` · `plan_words` · `observed_words` ·
  `watch_note` · `cannot_act` · `improvement_note` · `crew_hash` ·
  `turned_fact`; the five mcp kinds ported — the Python suite reads 361 (336
  before: 23 fixture cases, the beat-lock test, W26's test);
  Rust **ported 195/195 cases, 49 kinds, not yet ported: none** (157/172
  before). `cargo test` green across the workspace; clippy clean with and
  without the features; the five sacred crates and `orrethd` untouched; no
  new workspace deps.
  **THE SHADOW PROOF OF THE LOOPS** (`tests/loops.rs`, rig-gated, 28 s): the
  Python rig WHOLE but for its dispatcher (its scheduler and intent loops
  alive) + the Rust bridge in-process, both beating on one ground — a
  schedule every 5 s through the Rust door occurred once per cadence, never
  two occurrences within 4.5 s, though two tickers beat; a watch born red
  (`bodies_alive >= 0`) turned ONCE — one `watch.turned` fact, one `watch-red`
  marker under Resiliency, one plan to the planner (the Python FakeGateway),
  one objective filed to the librarian in Resiliency's session — and a
  standing red was never observed again; `/monitor` (watches) · `/schedules/
  echo` · `/harness` · `/intentions` · the objective's `/ask` read EQUAL on
  both doors; the occurrence wore W21's duty framing; a human intention
  declared through `/ask` was stopped through `/intentions/stop` (held
  L3-code; a wrong code 403; the right code settled it — `intent.stop` ran
  in the Rust kernel, the reply "Done, on your code: the intention … is at
  rest"), restarted the same way (history whole: `stopped_by` and
  `restarted_by` both stand); the kernel's Resiliency stop held for
  code-then-master and was cancelled. The sp3 proofs re-run green
  (`askroad` now reads the Analyzer until both doors agree — a body still
  acquiring at boot moved a count between two reads).
  **Honest limits:** the Rust kernel holds no bodies, so the harness's A/B
  RUN (a mind's) and the kernel's scheduled harness duty stay the Python
  kernel's (`/harness/run` 501; the duty left due); the toolkeeper's beat
