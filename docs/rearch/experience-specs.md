...
| **SPEC-SCHED-01** | P16 | Open a resident's card | Every schedule it runs is listed — human, role, and kernel; the kernel-critical one refuses edit with plain words | expected FAIL | BUILT (P4 sp5) — the card lists human · role · kernel schedules with their beats; the kernel one refuses edit in plain words; a human one is scheduled and rested from the card (rest is recorded, never a delete) — walk OWED |
