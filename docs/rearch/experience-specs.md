...
| **SPEC-EXPORT-01** | AG-7 · P6 sp2 | After a planned request (plan over two residents' answers), say "export compliance for this session" | A table renders in place: every ask, act and reply with its chain H → resident → firmware end to end, its proof level and its WHY; a download offered; the chain is unbroken on the planned request | planned | — |
| **SPEC-MITL-01** | P6 sp3 · MITL | Say "summon MITL"; then, before cutting a watch or an intention, click "expected impact?" | MITL answers in the chat with who and what the change touches, the risk in words, what to watch after; the confirm that follows shows its answer beneath; MITL's cognition shows on the meter | planned | — |
| **SPEC-PLACE-01** | P10 · P6 sp4 | Open a crew card; read where the body stands; declare a template whose placement the ground cannot honor | The card says where and why; the impossible placement is refused at birth with the reason in plain words, never started | planned | — |
