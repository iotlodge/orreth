...
| Old organ (dive) | What it did | Seat | Evidence / gap |
|---|---|---|---|
| Services are identities (0018) | DID'd tools/MCPs, manifest hash pin, lifecycle ladder, drop = lease expiry, charlotte | CARRIED partial — `services.py` · `mcp.py` · `templates/firmware-toolkeeper.v0.json` (P6.5 sp1 + sp2) | ONE registry for tool · mcp · store · source · mind: a `did:orreth:service:…` self per service (seed persisted, rule 1), the manifest pin, the ladder registered → versioned(n) → healthy \| unhealthy → retired (restore a new fact), every step a fact with chain + marker; the built-ins registered at boot; the meter and the tool hop carry the service DID; **the keeper stands** (sp2: `toolkeeper`, charlotte's seat as a third-kind firmware body — registers · checks · versions · retires · restores on the human's word through the `services` tool at the door, holds at the interlock, PROPOSES after N strikes and never retires alone); an MCP server and each of its tools are selves on the ladder (a changed list versions; a vanished tool goes unhealthy "gone from the server's list"). **Missing:** drop = lease expiry (a service holds no lease yet), quarantine on a changed manifest (a change VERSIONS today), allocations per body/world. |
...
| MCP as a whole transport (0059) | `tools/call` through the one door, session, SSE | CARRIED — `mcp.py` (P6.5 sp2) | the kernel's MCP client, JSON-RPC 2.0 over stdio (a spawned command) and streamable HTTP (a URL; SSE answers unwrapped): `initialize` · `tools/list` · `tools/call`, a session per act; a server registers as kind `mcp` with its locator BY NAME and its listed tools land as kind `tool` under it; the ToolDoor dispatches an MCP-born tool through `tools/call` — journaled with the service DID and the chain (AG-7), metered, the result whole; `destructiveHint` → consequential (the interlock). **Missing:** a standing session (every act re-initializes), OAuth, resources/prompts (tools only), the seed catalog. |
| The env-secrets law (0059) | `env:NAME` indirection; a key in ZERO records | CARRIED — `services.py` `register(secrets_with=…)` · `placement.py` (P6.5 sp1) | a service names its secrets by NAME; register refuses BY NAME when the ground cannot reach one and records nothing; the shelf shows ✓/✗ by name; a value is printed nowhere. P6.5 sp2: an MCP server's child environment is built from NAMES at spawn (`mcp.spawn_env`: PATH and the named secrets, nothing else), an HTTP bearer rides a named secret as a header, `env:NAME` names even the locator; a server whose named secret is unreachable refuses BY NAME before anything is spawned, and records nothing. |
...
**Tally (63 rows, re-counted 2026-09-23 after P6.5 sp2 — the MCP row moved RE-SEATED → CARRIED):** CARRIED 10 · CARRIED partial 13 · RE-SEATED 26 ·
