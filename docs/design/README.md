...
| 0004 | Tier Profile | What memory + objective vector + **time-budget** mean per tier. **Head start:** every locked dial already ships as `contracts/v0/tier-profile.schema.json` — the narrative spec will refine, not redefine. | ⏳ |
...
| **0013** | [The Custodian Tier & Responsible-Universe Architecture](0013-custodian-tier-and-responsible-universes.md) | The dual-use reckoning, made structural. Custodian = apex-above-apex (freeze-not-read) · platform floors (meta-cascade) · **Warden** resident organ (**detection, not enforcement**) · graded quarantine w/ **HITL placement map** · key custody as the fork · trust-tiered onboarding · abuse taxonomy · POCs (Earth Mapper, Agentic Enterprise). **Reconciled by Fable 5** — added the HITL rigor the earlier pass missed; §12 decisions **reopened** (were never JB-locked). Amends `0000`. | 🟡 draft for review |
