...
| 2 | **The passage** — the worker's silence watch (reach-out → seal, reversible, loud) · the heartbeat unseal · the attestation door (0029 evidence artifact + entitled attestations → staged escalation, quorum + cooling-off per the lock) | ✅ landed 2026-07-17 — `continuity.py` grows §3's machine as pure law: `PASSAGE_STATES` + the legal-edge table (`may_transition` — no shortcut past the reach-out, nothing leaves the closed states, only SEALED reaches attestation) · `silence_verdict` (living < window ≤ unresponsive < 2× ≤ sealed, NEVER more — lock 1; a testament-day scales by a rig dial, never law) · passage records ride one worldline, every transition naming its trigger · `ATTESTATION_COOLING_DAYS = 7`, probate-shaped (design-owner call, JB may veto) · becky's doors: "show the passage" speaks the state · "attest death:" STAGES with evidence + roster readable (never on words alone) · "abort the attestation" acts NOW — one voice saves · worker `passage_beat` sweeps every floor with a standing testament; the glass agate wears red: "start the cooling-off" · **proven as a human on u:demo, the whole arc**: the watch sealed the quiet universe unstaged on its first sweep → one heartbeat unsealed it ("nothing was lost") → revised the word to 6 days at the gate → silence → reach-out → sealed → attested at the gravest gate (quorum 2, cooling until on the record) → **spoke during the cooling-off and the machine wrote "a heartbeat aborted the attestation — the loudest abort"** · TWO LIVE CATCHES, both the same organ: the ranked retrieve trails and subsets the record, so (a) the beat now trusts its own hand over the stale mirror (`_PASSAGE_HAND`/`passage_now`, every door consults it — the 0032 dedup lesson, third organ to learn it; reach-outs never double), and (b) **silence must be OBSERVED, never inferred** — the watch floors its verdict at its own boot and at the worker's first-hand record of answering a living human; a dedicated tag-indexed read is the queued improvement. 4 tests, 180/180 |
| 3 | **The execution walks** — per domain: seal-standing · succession grant springs + graft-with-lineage · governed shred through 0026's doors · the worldline close, the portrait frozen | ✅ landed 2026-07-17 — the gravest code, and it walked: `continuity.py` grows the heir (a domain that passes REFUSES at mint without one — custody must have hands to land in) · `execution_walk` renders the fate map into steps, pure (heirless pass degrades to seal loudly · a shred under legal hold QUEUES, it does not die (0004) · key mortality named as its own method · the unnamed seal closes every walk) · `succession_terms` springs ONLY at the close — custody never identity: retrieve + graft over the pass domains, never govern, never the keys, never the voice · `make_graft` — continuation by graft: `derived_from` crossing universes, tag `inherited`, the parent whole · `sovereign_alive` freezes the portrait structurally: the librarian's assert door refuses past EXECUTED · worker: cooling expiry in silence → `execute_estate` — EXECUTED, then one becky-signed **estate record per step**, then LEGACY · **proven as a human on a universe grown to die**: the shipyard laid u:demo/e:estate/f:probe (continuity template) → seeded two journals + one identity → the fatal word at ITS becky (2-day window) → sealed in silence → attested → the cooling-off passed unspoken → the walk fired live: *identity → custody springs for the heir · journals → 2 record(s) through the tombstone door, stubs survive · medication → sealed standing · the unnamed sealed* → **LEGACY — "the walk is complete — the universe keeps"** · becky speaks it: "the archive speaks about, never as" · the librarian refuses the assert: "the portrait is frozen — the sovereign class died with its sovereign" · u:demo untouched, its own resting testament re-staged heir-complete; the dead universe stays in the orrery, kept. 4 tests, 184/184 |
| 4 | **The survivors' door** — the legacy register in the label canon · parlor read-only for the entitled · the legacy Brain Glass (the constellation) · hibernated economics | ✅ landed 2026-07-17 — **0035 IS WHOLE, the dive closes, the era turns to v0.35**: `speak_legacy` — the label canon's last row, structural ("the record holds: …", "the record suggests, unproven: …", "the record was re-checking this when it closed: …"; the recalled stay unspoken, doubly; no shape opens in first person) · `survivors_door` composes who may still read from the word itself: custody first, the disclosure map's named doors second, the honest close for the rest ("the universe keeps, unread — grief is not an entitlement") · **the read-only law**: one sentence at every mutating door on a legacy floor — "the archive is read-only: it keeps, it never spends, and nothing new enters" (becky's grants/testaments/growth · the librarian's asserts/gathers/subscriptions/challenges) while every read door stays open — legacy is not a tomb with the lights cut · **the organs rest** (hibernated economics, 0009's law's final application): the Mirror never assesses the dead, the desk never spends there; a legacy floor always carries its settled `legacy` marker, healed idempotently, so the glass knows · **the legacy Brain Glass**: no golden fire, no signals, no bursts — the anatomy whole, embers cooled to a silver-blue constellation, the mind turning slowly, captioned "the shape of a mind, held" · **proven as a human on the standing grave f:probe**: becky's survivors' door spoke custody for the heir verbatim · the librarian refused a subscription with the one sentence · the brain rendered the constellation · **the two-brains contrast gained its third: the enterprise burns, the continuity mind breathes, the legacy keeps.** 3 tests, 187/187 |
...
*Built and proven 2026-07-17, all four spoonfuls in one day, on a universe grown
to die: u:demo/e:estate/f:probe lived twenty-two minutes, spoke its word, went
quiet, was attested with gravity, walked its fates to the record, and keeps —
a constellation in the orrery, readable exactly as it chose. 187/187.*
