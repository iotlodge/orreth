...
```mermaid
flowchart LR
  W["UAT personas walk<br/>REAL objectives daily"] --> R["friction receipts<br/>the THUMB — every 👎 quotes the persona"]
  R --> T["the architect triages (J3):<br/>defect · craft edit · design change · named gap"]
  T --> G["JB's gates —<br/>his ONLY seat this season"]
  G --> S["better sentences · views · flows<br/>(cards-as-craft, released + versioned)"]
  S --> W
  M["the mentor lane:<br/>skills I build → crystallized →<br/>worn by cheaper minds (0039)"] -.-> S
```
