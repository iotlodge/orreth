...
| 2 | **The aperture** — assembled + signed at dispatch; the chassis pins it; the ledger shows it | ✅ landed 2026-07-15 at the Phase D gate (JB's approvals, per change): `fingertip.make_aperture` — the whole opening (of · seat · agent · law · task · behavior pins · knowledge refs), everything by reference, signed by the DISPATCHING seat, content-addressed, carrying its own hard coordinate · the sim Flow cuts one per intention (the law = the resolver's deterministic address — same cascade, same hash) and the chassis pins every RunRecord to it (`context_hash` semantics widened, JB-approved); the wire's orchestrator cuts one per fanned leg (the wire's law = the human-approved plan record; behavior = the shelf's active version — the shelf speaks at dispatch) · the socket check passed: JB's data-scheme reveal WAS the information-theory ground (0033), and the aperture landed against the hardened coordinate exactly as designed · proven live: one apex objective fanned 7 intentions → 7 signed apertures on u:demo's record. 171/171 |
