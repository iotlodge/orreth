...
4. **The retirement of `curate_plan()`**: the arithmetic planner becomes the
   ...
   card, never silent. **LOCKED (JB, 2026-08-07, AskUserQuestion): the label
   wears the as-built wording** — "planned by arithmetic, not a mind — the
   budget split evenly across the floors; the studio has not answered".
...
| 3 | **The studio comprehends** — the mind's first duty: Objective → typed Understanding, read from the live registry by reference; the plan card carries it · **LANDED 2026-08-07**: `agents/flavors/03-mind/run.py` — the studio, a citizen mind wearing the sp2 jacket (joined at becky's gate, benched at f:prod where the medium mind serves — the judge-bench pattern, law 2's shape; the SEAT stays the universe's queue); `understand-objective` firmware genesis-seeded (⟦objective⟧ ⟦rubric⟧ ⟦registry⟧); every arriving Objective posts an `understand` leg (0027 §2 — dispatch is a governed request) and the card stages INSTANTLY — the gate never waits on the studio; `studio_tend` attaches the reading when the mind answers, labels the silence dark past 90s, and a late reading still upgrades a standing card; the glass renders all four states (read · reading… · dark · parked) on the plan card | **PROVEN LIVE, BOTH WAYS, the cards left at JB's gate**: req-476 ("summarize retail week + translate to Spanish") staged wearing the full reading — grounded in a REAL registry name (`skill-summarize-a-floor-s-week`), **the gap named unprompted: "no translation skill or prompt is present in the registry"** (law 7's detector alive — sp5's GapReport shape flowing), confidence 0.45 honest, craft + studio DID pinned; req-478 staged with the studio resting: "the studio is dark — staged without a reading" after the deadline, never blocked · the studio metered under its own fueled lease: 2,291 tok / $0.00943, remaining 47,709 — **and calls:2 vs runs:1 — the sp1 typed re-ask FIRED LIVE and the reading landed on the second dressing**, a word the old regime would have LOST · **the glass confirmed by JB's own screen** (2026-08-07): both cards rendering at the gate — the reading with its gaps in amber, the dark card's honest absence beside it — ✓ RAN |
| 4 | **The planner authors** — plan() returns GraphSpec through `check()`; the fingertip executes it; `curate_plan()` demoted to labeled fallback · **LANDED 2026-08-07**: `fingertip.author_plan()` — **the mind proposes, the law disposes**: semantic decomposition is the studio's; pricing, entitlement marking, and the save gate stay deterministic (money is never the mind's to invent); the accepted plan carries `spec` — the workflow template's hash, `check_workflow`-refused-at-save; the draft chain rides the queue (reading → `plan-draft` leg with the legal seats → the save gate → the card SWAPS or the fallback stands labeled); **the card is the truth the human approved** (approval fans the card's plan, always — rule 7 closed at the gate); the studio's `plan()` wears a callable contract so a badly dressed draft earns the jacket's re-ask before the gate ever sees it; every card now DECLARES its planner (mind · drafting · fallback · refused · parked — five states in the glass); suite 304→306 | **PROVEN LIVE, ALL THREE WAYS**: (1) the SAME objective planned by both regimes — arithmetic: 18 identical copy-paste legs to every floor incl. e:care and the estate; **the mind: 8 legs, all on the RAG ecosystem, the umbrella seat told to synthesize and each flavor floor told to report its own standings, per-seat sentences, richer slices (266 vs 126 tok)** — spec sha256:1421b2b0… on the card, fallback retired, approved and FANNED AS PLANNED: 8 branches assembled, the request resolved; (2) **refused-at-save live**: a ghost-seat plan hand-fed at the wire died at the gate — "no floor named u:demo/e:ghost stands on this wire — refused at save" — the card wears the refusal and the fallback's legs still stand; (3) the honest-failure lane fired UNPROMPTED: the first live draft TRUNCATED at the studio's 400-token ceiling, failed its contract twice, and PARKED with the fallback standing labeled — the ceiling raised to 800 with the lesson in a comment — ✓ RAN · **lock 4 LOCKED as built (JB's word, same day)** |
...
- **The poisoned ledger** (found 2026-08-07, sp2's live-fire): the plane's
  ...
  for JB's explicit word on the code**), and **JB's architecture guidance
  landed 2026-08-07**: budgets become a managed lifecycle, visible in
  «Governance» — BOTH mechanisms (re-initialize on a newly VERIFIED larger
  token, and replenishment) because each alone fails (auto-replenish is an
  abuse vector; re-init alone starves honest work), **bounded by an
  escalation law: more than twice in X period → HITL required, and the
  intent/observation WAITS at the gate until the human decides**. To be
  architected as its own small design when the fix session opens the crate.
