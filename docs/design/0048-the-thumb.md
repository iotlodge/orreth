...
```mermaid
flowchart TB
  R["a resident's reply (0020 parlor)<br/>or a resolved Objective (0030 ledger)"] --> TU["👍 — one click, fades from view"]
  R --> TD["👎 — the dialog opens, the human's text"]
  TU --> HV["signed HUMAN VERDICT<br/>grader = the human's seat DID ·<br/>judges ONE record by hash ·<br/>lands on 0043's human-grading shelf"]
  HV --> CAL["calibration: human vs examiner on the same work —<br/>disagreement is NEWS: a staged finding,<br/>min-n before it speaks, cards never levers"]
  TD --> FB["FEEDBACK RECORD (0024 marker grammar)<br/>derives from what it marks · severity-laned · signed"]
  FB --> READ["the studio READS it (0047)<br/>the lack, typed — one re-ask, then parks honestly"]
  READ --> C1["craft defect →<br/>sibling proposal (0028/0031/0045)"]
  READ --> C2["knowledge/skill gap →<br/>🌱 commission (0047 §6)"]
  READ --> C3["charter mismatch →<br/>referral / charter card (0046)"]
  READ --> C4["execution failure →<br/>repair objective (0027 · 0030)"]
  C1 & C2 & C3 & C4 --> GATE["the human's gate (0012)<br/>improvement proposed, NEVER self-adopted (0031)"]
  GATE --> CLOSE["CLOSURE — the request RESOLVES:<br/>the outcome named back to the thumb's author<br/>(the card always; the bell only under standing consent, 0044)"]
```
