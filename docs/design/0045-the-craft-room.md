...
*Drafted 2026-08-03 · Fable 5 with JB · status: **CLOSED WHOLE 2026-08-03,
era 0.45** — designed, blessed, and all six spoonfuls proven in one day*
...
| 5 | **The supply line** — the SDK acquire door (law 8): resolve craft by name under a lease, head or pinned, arm-serving when an argument stands, servings on the record; the applied graph gains EXTERNAL wearers · **LANDED 2026-08-03**: `orreth_agent/craft.py` (stdlib-light `acquire()` → ResolvedCraft — one resolution carried through the run; ⟦slot⟧ rendering; the citizen DID; `on_dark` declared) + the worker's `/craft` door (head or pinned — a pin off the worldline refuses; arm drawn by DID hash when an argument runs, VISIBLE in the reply — no secret splits; every serving on the instrument tier, servings.jsonl, the flight recorder's precedent) · **PROVEN**: a REAL LangGraph StateGraph (three nodes, its own shape) acquired assay-judge and its run record named the exact word — v2, the sha of THE RELEASE JB CUT — citizen persistent, arm field visible; then the registry was darkened mid-supply and the flow got the cached signed copy LABELED `stale: true`. The arm proof waits, fittingly, on JB's word at the standing argument (req-418). Lease enforcement at this door = named hardening gap | a real LangGraph flow acquires a governed prompt by reference — its shape unchanged; the run's record names the exact version — ✓ RAN (the arm rides JB's word) |
| 6 | **The commission** — create-button → objective → librarian gathers → factory builds → grace stages → the bell rings → v1 in the registry · **LANDED 2026-08-03, THE DIVE CLOSES WHOLE**: `on_commission` — the human's click births craft through the machine's own organs: the librarian gathered through the live tavily stall, the factory drafted at a CHOSEN bench (metered, with one honest redraft when the first came back empty — printed, never silent), grace landed v1 with `commissioned_by` and `born_of` named, and the card waits at the gate wearing "welcome it — seen" (in-glass bell; out-of-band commission rings await a consent widening — one row, behind consent, law 3 kept) · **PROVEN**: `skill-orreth-langgraph-onboard` v1 stands in the registry — the first wire-born skill of the universe floor, its whole birth walkable; the welcome click left standing for JB's own hand. Era → 0.45 | JB commissions a real skill from the glass and is rung when it stands — ✓ RAN, the welcome his |
