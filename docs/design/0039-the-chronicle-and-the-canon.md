...
| # | Spoonful | Nature |
|---|---|---|
| 1 | **The two books stand** — record classes gain charter attributes; the Canon census (every existing shelf asset formally a Canon entry); the projection privacy floor enforced by class | the constitution lands |
| 2 | **The Chronicle joins the rows** — ladder records retrievable through the stacks (class-gated, trust intact); TIME as a Dispatcher shape ("as of…", "what changed since") — the spacetime window and the stacks join hands | the universe remembers itself |
