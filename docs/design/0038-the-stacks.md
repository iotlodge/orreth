...
| # | Spoonful | Nature |
|---|---|---|
| 1 | **The stacks stand** — e:rag provisioned through allen's door (the Objective on record); eco assets planted (chunking · embedding standard · prompt templates); `f:naive` end-to-end over the one truth: ingest once → project → retrieve → answer with citations | the baseline breathes |
| 2 | **The Dispatcher** — the organ in the put/get path; the routing standard v1 as an asset; every choice a record; the librarian's escalation seat | the reflex and the judgment |
| 3 | **The rivals** — `f:rerank` · `f:graph` (pg projection) · `f:hybrid`; same asks, competing receipts | the field takes shape |
